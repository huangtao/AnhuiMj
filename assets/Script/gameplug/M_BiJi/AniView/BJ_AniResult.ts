import AniResultType from "./BJ_AniResultType";
import AniChip from "./BJ_AniChip";
...
import { CardsCount } from "../GameHelp/BJ_GameHelp";
...
export default class AniResult extends cc.Component {
     ...
     * 结果消息
     */
    private msg_gameResult: M_BiJi_GameMessage.CMD_S_GameResult;
    /**
     * 计分板消息
     */
    private msg_scoreView: M_BiJi_GameMessage.CMD_S_ScoreView;
    /**
     * 庄家变更消息
     */
    private msg_masterChange: M_BiJi_GameMessage.CMD_S_MasterChange;
    /**
     * 玩家记分
     */
    private msg_playerScore: M_BiJi_GameMessage.CMD_S_PlayerScore;
    ...
    public Destroy() {
        ...
        if (this.msg_gameResult != null) {
            BiJi.ins.iview.Rec_GameResult(this.msg_gameResult, true);
            this.msg_gameResult = null;
        }
        if (this.msg_scoreView != null) {
            BiJi.ins.iview.Rec_ScoreView(this.msg_scoreView, true);
            this.msg_scoreView = null;
        }
        if (this.msg_masterChange != null) {
            BiJi.ins.iview.Rec_MasterChange(this.msg_masterChange, true);
            this.msg_masterChange = null;
        }
        if (this.msg_playerScore != null) {
            BiJi.ins.iview.Rec_PlayerScore(this.msg_playerScore, true);
            this.msg_playerScore = null;
        }
    }
    /**
    ...
    public ShowAni(data: M_BiJi_GameMessage.CMD_S_GameResult) {
        ...
        this.schedule(this.ShowResultCards, 0.8 * BiJi.ins.iclass.GetSpeed());
    }
    /**
     * 亮牌
     */
     ...
     * 在动画播放完毕时再显示计分板
     */
    public SetScoreView(data: M_BiJi_GameMessage.CMD_S_ScoreView) {
        if (this.node.active) {
            this.msg_scoreView = data;
        }
        else {
            BiJi.ins.iview.Rec_ScoreView(data, true);
        }
    }
    /**
     * 设置玩家变更消息
     * 同上
     */
    public SetMasterChange(data: M_BiJi_GameMessage.CMD_S_MasterChange) {
        if (this.node.active) {
            this.msg_masterChange = data;
        }
        else {
            BiJi.ins.iview.Rec_MasterChange(data, true);
        }
    }
    /**
     * 设置玩家余额
     * 同上
     */
    public SetPlayerScore(data: M_BiJi_GameMessage.CMD_S_PlayerScore) {
        if (this.node.active) {
            this.msg_playerScore = data;
        }
        else {
            BiJi.ins.iview.Rec_PlayerScore(data, true);
        }
    }
    /**
     * 显示玩家牌结束
    ...
    private AniShowCardsOver() {
        ...
        if (index >= 0) {
            ...
            this.scheduleOnce(this.AniShowCardsOverFun, 0.5 * BiJi.ins.iclass.GetSpeed());
        }
        else {
    ...
    private AniShowChipOver() {
        ...
        if (this.msg_playerScore != null) {
            ...
            BiJi.ins.iview.UpdatePlayerScoreByGameResult(this.msg_gameResult);
        }
        this.AniOver();
    }
    /**
    ...
    private GetCardsByIndex(value: number) {
        ...
        for (var i = 0; i < CardsCount; i++) {
            cards[i] = this.msg_gameResult.headcards[value * CardsCount + i];
        }
        return cards;
    }
