import { TingCardTip } from "../ConstDef/TDHMJMahjongDef";
import M_TDHMJClass from "../M_TDHMJClass";
import TDHMJ_SingleTingCardTip from "./TDHMJ_SingleTingCardTip";

const { ccclass, property } = cc._decorator;

@ccclass
export default class TDHMJ_TingTip extends cc.Component {
    ...
    @property(cc.Prefab)
    TDHMJ_SingleTingCard_Tip: cc.Prefab=null;
    ...
    //结算
    private _singleTingCard: TDHMJ_SingleTingCardTip;
    /**
     * 结算视图
     * */
    public get SingleTingCard(): TDHMJ_SingleTingCardTip {
    ...
    //听牌提示
    private _tingCardAry: Array<TDHMJ_SingleTingCardTip>;
        ...
        if(this._tingCardAry!=null)
        {
            while (this._tingCardAry.length > 0) {
               // this._tingCardAry.pop().node.destroy();
                TDHMJ_TingTip._freeNode.put(this._tingCardAry.pop().node);
            }
        }else{
            this._tingCardAry = new Array<TDHMJ_SingleTingCardTip>();
        }
    ...
    public showTingTip(tingTip: Array<TingCardTip>,tips:boolean): void {
       ...
       // M_TDHMJView.ins.TingTip.node.x=-330;
        ...
        for (var i: number = 0; i < tingTip.length; i++) {
          let singleTip = TDHMJ_TingTip._freeNode.get();
            if (!cc.isValid(singleTip)) {
                singleTip = cc.instantiate(this.TDHMJ_SingleTingCard_Tip);
            }
            this._singleTingCard=singleTip.getComponent<TDHMJ_SingleTingCardTip>(TDHMJ_SingleTingCardTip);
