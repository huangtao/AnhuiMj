...
import TDHMJ_SinglePlayerRecordX from "./TDHMJ_SinglePlayerRecordX";

const { ccclass, property } = cc._decorator;

@ccclass
export default class TDHMJ_JiFenBanX extends cc.Component {
    ...
    public SetPlayerData():void{
        for(var i:number=0; i<TDHMJMahjongDef.gPlayerNum; i++){
            LoadHeader(TDHMJ.ins.iclass.getTablePlayerAry()[i].FaceID, this.img_accountAry[i]);
    ...
    private loadRecordData():void{
        ...
        for(let i: number = 0;i < this._gameRecordAry.length;i++) {
             let newnode = TDHMJ_JiFenBanX._freeNode.get();
            if (!cc.isValid(newnode)) {
                newnode = cc.instantiate(this.single);}
           // let newnode=cc.instantiate(this.single);
            let sin=newnode.getComponent<TDHMJ_SinglePlayerRecordX>(TDHMJ_SinglePlayerRecordX);
