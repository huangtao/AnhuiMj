...
@ccclass
export default class BBMJ_UpVideoActive extends BBMJ_VideoActiveBase {
    ...
    protected refreshHandCard(): void {
        //起始位置
        var startPos: number = BBMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];

        //开始排版
        
        if(BBMJ.ins.iclass.is2D()){
            //起始位置
            let startPos: number = BBMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];
