...
import MGMJ_SinglePlayerRecord from "./MGMJ_SinglePlayerRecord";
import M_MGMJClass from "../M_MGMJClass";
import MGMJ_HeadPortrait from "./MGMJ_HeadPortrait";

const { ccclass, property } = cc._decorator;

@ccclass
export default class MGMJ_JiFenBan extends cc.Component {
    @property(cc.Button)
    ...
    private _touxiangAry:Array<MGMJ_HeadPortrait>;
    ...
    private InitTouXiang():void{
        this._touxiangAry=new Array<MGMJ_HeadPortrait>();
        for(var i=0;i<this.pfb_touXiang.length;i++){
            var toux=cc.instantiate(this.pfb_touXiang[i]);
            this._touxiangAry.push(toux.getComponent<MGMJ_HeadPortrait>(MGMJ_HeadPortrait));
    ...
    onLoad() {
        ...
        protected onShow(): void {
            ...
            for(var i:number=0; i<MGMJMahjongDef.gPlayerNum; i++){
                ...
                //this._img_accountAry[i].source=MGMJ.ins.iclass.getTablePlayerAry()[i].FaceID;

                this._touxiangAry[i].setHeadPortSpri(MGMJ.ins.iclass.getTablePlayerAry()[i].FaceID);
        ...
        private loadRecordData():void{
            ...
            for(var i: number = 0;i < this._gameRecordAry.length;i++) {
                ...
                // record.node.y = i * 61;
            //       let newnode = MGMJ_JiFenBan._freeNode.get();
            // if (!cc.isValid(newnode)) {
            //     newnode = cc.instantiate(this.single);
            // }
            //   let sin=newnode.getComponent<MGMJ_SinglePlayerRecord>(MGMJ_SinglePlayerRecord);
        ...
        private AddDataToContain(data:M_MGMJ_GameMessage.GameRecordResult,i:number){
         //   var dt=cc.instantiate(this.pfb_SinglePlayerRecord);
          ...
          //  sin.init(this._gameRecordAry[i],i);
            ...
            newnode.y=i*61;
            var xx:MGMJ_SinglePlayerRecord=newnode.getComponent<MGMJ_SinglePlayerRecord>(MGMJ_SinglePlayerRecord);
            xx.Init(data,i);
