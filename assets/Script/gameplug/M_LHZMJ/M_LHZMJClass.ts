...
* */
  ...
  */
    ...
    onLoad() {
        ...
        this.two_dimensional = this.getCurSceneCanvas() == "2D"?true:false;
    ...
    */
    ...
    public OnGameMessage(cm: GameIF.CustomMessage): boolean {
        switch (cm.wSubCmdID) {
            ...
            //开始之前发起三人开始之后有玩家离开
            case M_LHZMJ_GameMessage.LHZMJMsgID_s2c.CMD_S_AfterThreePlayUserLeftMsg: {
                this.Handle_CMD_S_AfterThreePlayUserLeftMsg(cm);
                break;
            }
            //玩家断线
 ...
 */
    ...
    protected OnPlayerSitDown(chairID: number, player: QL_Common.TablePlayer): void {
        ...
        if (playerCount <= 3 && this._userNumRule == 4) {
            ...
            if (playerCount <= 2) {
                // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.color = cc.color().fromHEX("#B8B8B8");
                // this.gameView.ReadyAndGameUserInfo.lab_threeBodyPlay.node.color = cc.color().fromHEX("#B8B8B8");
                // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.getComponent(cc.Button).interactable = false;
                this.gameView.ReadyAndGameUserInfo._comeInCont = true;
            } else {
                // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.color = cc.color().fromHEX("#FFFFFF");
                // this.gameView.ReadyAndGameUserInfo.lab_threeBodyPlay.node.color = cc.color().fromHEX("#FFFFFF");
                // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.getComponent(cc.Button).interactable = true;
                this.gameView.ReadyAndGameUserInfo._comeInCont = false;
                let action= cc.repeatForever(cc.sequence(cc.scaleTo(0.5, 0.9), cc.scaleTo(0.5, 1.2)));
                this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.runAction(action);
            }
            // this.gameView.ReadyAndGameUserInfo.btn_ready.node.active = false;  
        }
        if(this._userNumRule == 3){
            this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.active = false;
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (this._userNumRule == 4) {
                ...
                if (playerCount < 4) {
                    ...
                    // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.getComponent(cc.Button).interactable = false;
                    this.gameView.ReadyAndGameUserInfo._comeInCont = true;
                     this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.stopAllActions();
                } else{
                    // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.color = cc.color().fromHEX("#FFFFFF");
                    // this.gameView.ReadyAndGameUserInfo.lab_threeBodyPlay.node.color = cc.color().fromHEX("#FFFFFF");
                    // this.gameView.ReadyAndGameUserInfo.btn_threeBodyPlay.node.getComponent(cc.Button).interactable = true;
 ...
 * */
    ...
    protected OnNetResponding() {
        ...
        //显示桌布
        this.gameView.Init();
        M_LHZMJView.ins.TimerView.showLuoPan(this.SelfChair);
        this.regTimer(LHZMJTimerDef.timer_id_playerop,0,this.SelfChair);
         // M_LHZMJView.ins.TimerView.showLuoPan(this.sel);
        // M_LHZMJView.ins.TimerView.ArrowNode.node.active = false;
    }
    ...
    **/
    ...
    private Hanle_CMD_S_AddGameNum(msg: GameIF.CustomMessage): void {
        ...
        if (addGameNum.gameNum == 100) {//所有玩家同意续局
            cc.log("-----全部同意续局-----");
            M_LHZMJView.ins.ReadyAndGameUserInfo.btn_threeBodyPlay.node.active = false;
     ...
     * 游戏开始之前发起三人玩法后有人离开
     * */
    private Handle_CMD_S_AfterThreePlayUserLeftMsg(msg: GameIF.CustomMessage): void {
        var playerOffLine: M_LHZMJ_GameMessage.CMD_S_AfterThreePlayUserLeftMsg = <M_LHZMJ_GameMessage.CMD_S_AfterThreePlayUserLeftMsg>msg;
        // playerOffLine.chair;
        M_LHZMJView.ins.TipMsgShow("有玩家离开，三人开始玩法投票失效，请重新发起申请！");
    ...
    public getMahjongPaiHua2D(card: number): cc.SpriteFrame {
        return this.paibei.getSpriteFrame(`mahjong_${LHZMJMahjongAlgorithm1.GetMahjongColor(card)}_${LHZMJMahjongAlgorithm1.GetMahjongValue(card)}`);
    }
