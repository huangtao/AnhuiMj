...
import TDHMJ_SingleActive from "../single/banlanceShow/TDHMJ_SingleActive";
import { ReportError } from "../../../../Tools/Function";

const { ccclass, property } = cc._decorator;

@ccclass
export default class TDHMJ_BanlanceActive extends TDHMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<TDHMJ_SingleActive>;
    /**
     * 手牌
     * */
    ...
    public init() {
        this._cardData = new Array<TDHMJ_SingleActive>(0);
    ...
    protected refreshHandCard(): void {
        ...
        for(var i: number = 0;i < this._cardData.length;i++) {
            ...
            if(this._holdCard != TDHMJMahjongDef.gInvalidMahjongValue && (i == (this._cardData.length - 1))) {
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(var i: number = 0;i < newNum;i++) {
              ...
              let newnode = TDHMJ_BanlanceActive._freeNode.get();
                if (!cc.isValid(newnode)) {
                    newnode = cc.instantiate(this.CardType);
                }
                this.node.addChild(newnode);
                var active = newnode.getComponent<TDHMJ_SingleActive>(TDHMJ_SingleActive);
                active.init();
            ...
            for(var i: number = 0;i < delNum;i++) {
             //   this._cardData[i].node.destroy();
              TDHMJ_BanlanceActive._freeNode.put(this._cardData[i].node);
            }
    ...
    public clear(): void {
        ...
        while (this.node.children.length > 0) {
            ...
            if (cc.isValid(active)) {
                ...
                //     active.destroy();
                // } else {
                    TDHMJ_BanlanceActive._freeNode.put(active);
                // }
            } else {
                ReportError("手牌数据出错！！！");
            }
