...
@ccclass
export default class TDHMJ_SinglePlayerBalance extends cc.Component {
    ...
    @property(TDHMJ_BanlanceFixed)
    fixedCard: TDHMJ_BanlanceFixed=null;
    @property(TDHMJ_BanlanceActive)
    handCard: TDHMJ_BanlanceActive=null;
     ...
     * 显示玩家结算信息
     * */
    public showBalance(account:string,allcard:M_TDHMJ_GameMessage.PlayerCard,balance:M_TDHMJ_GameMessage.PlayerBalance,faceID:string):void{
     ...
     //   this.handCard.node.removeAllChildren();
        ...
        let ss:string = "";
        if(TDHMJMahjongAlgorithm.strLen(account) > 4){
        ...
        TDHMJMahjongAlgorithm.sortCardAry(allcard.handCard);
        if(allcard.huCard!=TDHMJMahjongDef.gInvalidMahjongValue)
            allcard.handCard.push(allcard.huCard);
        this.handCard.refreshHandCardData(allcard.handCard);
        let url="";
        if(balance.JieSuan[0]>0){
            // url="gameres/M_TDHMJ/Texture/LaPaoZuo/zuo";
            ...
            // url="gameres/M_TDHMJ/Texture/LaPaoZuo/la";
            // SetTextureRes(url,this.img_la_0);
            // SetTextureRes(url,this.img_la_1);
        }
