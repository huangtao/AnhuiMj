import { BBMJMahjongDef, clsParseResult, enMahjongPattern, enTripleType, clsParseTriple, clsPairTripleWrapper, enFixedCardType, clsFixedCard } from "../ConstDef/BBMJMahjongDef";
import { BBMJMahjongAlgorithm1 } from "./BBMJMahjongAlgorithm1";


const { ccclass, property } = cc._decorator;

export class BBMJMahjongPattern {
        ...
        {
            if (activeCard.length != BBMJMahjongDef.gCardCount_Active)
            {
                return false;
            }
        ...
        public static ParseCards(srcActiveCard: Array<number>,srcFixedCard: clsFixedCard,parseResult:clsParseResult):boolean{
            ...
            for(var i=0;i<srcActiveCard.length;i++){
                if(BBMJMahjongDef.gInvalidMahjongValue != srcActiveCard[i]){
            ...
            BBMJMahjongAlgorithm1.sortCardAry(activeCard);

            //先检查是否可以胡牌
            if(!BBMJMahjongPattern.CheckIfCanHuCardArray(activeCard))
            {
                return false;
            }

            //先检查是否是特殊胡牌：七对子和国世无双
            if (BBMJMahjongPattern.CheckIsSpecialStruct(activeCard,parseResult))
            {
            ...
            BBMJMahjongAlgorithm1.spiltHandCard(activeCard, vWan, vTong, vTiao, vZhi);

            //排序一遍
            BBMJMahjongAlgorithm1.sortCardAry(vWan);
            BBMJMahjongAlgorithm1.sortCardAry(vTong);
            BBMJMahjongAlgorithm1.sortCardAry(vTiao);
            BBMJMahjongAlgorithm1.sortCardAry(vZhi);


            //4、对每个单色牌进行解析
            var pairTripleWrapper = new clsPairTripleWrapper();
            BBMJMahjongPattern.ParseSingleColorCard(vWan, pairTripleWrapper);	        //解析万
            BBMJMahjongPattern.ParseSingleColorCard(vTong, pairTripleWrapper);	        //解析筒
            BBMJMahjongPattern.ParseSingleColorCard(vTiao, pairTripleWrapper);	        //解析条
            BBMJMahjongPattern.ParseSingleColorCard(vZhi, pairTripleWrapper);	        //解析字
        ...
        public static CheckIfCanHuCardArray(srcAry:Array<number>):boolean
        {
            ...
            /*
                ...
                2.3)、对+刻/顺
            */
            if(BBMJMahjongPattern.IsHuShiSanLan(vSrc)){
                return true;
            }

            if (BBMJMahjongPattern.IsHuSevenPairStruct(vSrc))
            {
                return true;
            }
            
            if(BBMJMahjongPattern.IsHuNormalStruct(vSrc))
            {
                return true;
            }
            return false;
        ...
        public static IsHuShiSanLan(vSrc:Array<number>):boolean
        {
            ...
            //分拣牌
            BBMJMahjongAlgorithm1.spiltHandCard(card,vWanCard,vTongCard,vTiaoCard,vZhiCard);
        ...
        public static ParsePattern(parseResult: clsParseResult,have19Jiang: boolean): enMahjongPattern
        {
            ...
            // //检查十三烂
            // if(BBMJMahjongPattern.CheckMahjongPattern_ShiSanLan(parseResult))
            // {
            //     return enMahjongPattern.MahjongPattern_ShiSanLan;
            // }

            //检查清单调
            if(BBMJMahjongPattern.CheckMahjongPattern_QingDanDiao(parseResult))
            {
                return enMahjongPattern.MahjongPattern_QingDanDiao;
            }
            //检查清七对
            if (BBMJMahjongPattern.CheckMahjongPattern_QingQiDui(parseResult))
            {
                return enMahjongPattern.MahjongPattern_QingQiDui;
            }
            //检查将对
            if (have19Jiang && BBMJMahjongPattern.CheckMahjongPattern_JiangDui(parseResult))
            {
            ...
            //检查龙七对
            if (BBMJMahjongPattern.CheckMahjongPattern_LongQiDui(parseResult))
            {
                return  enMahjongPattern.MahjongPattern_LongQiDui;
            }
            //检查清大对
            if (BBMJMahjongPattern.CheckMahjongPattern_QingDaDui(parseResult))
            {
                return enMahjongPattern.MahjongPattern_QingDaDui;
            }
            
            //检查大单吊
            if(have19Jiang && BBMJMahjongPattern.CheckMahjongPattern_DaDanDiao(parseResult)) {
                return enMahjongPattern.MahjongPattern_DaDanDiao;
            }
            // //检查带幺九
            // if (have19Jiang && BBMJMahjongPattern.CheckMahjongPattern_DaiYaoJiu(parseResult))
            // {
            //     return enMahjongPattern.MahjongPattern_DaiYaoJiu;
            // }
            //检查小七对
            if (BBMJMahjongPattern.CheckMahjongPattern_XiaoQiDui(parseResult))
            {
                return enMahjongPattern.MahjongPattern_XiaoQiDui;
            }
            //检查清一色
            if (BBMJMahjongPattern.CheckMahjongPattern_QingYiShe(parseResult))
            {
                return enMahjongPattern.MahjongPattern_QingYiShe;
            }
            
            //检查对对胡
            if (BBMJMahjongPattern.CheckMahjongPattern_DuiDuiHu(parseResult))
            {
        ...
        {
            //同时满足清一色,大单调
            if (BBMJMahjongPattern.CheckMahjongPattern_QingYiShe(parseResult) && BBMJMahjongPattern.CheckMahjongPattern_DaDanDiao(parseResult))
            {
                return true;
            }
            return false;
        ...
        {
            //同时满足清一色,小七对
            if (BBMJMahjongPattern.CheckMahjongPattern_QingYiShe(parseResult) && BBMJMahjongPattern.CheckMahjongPattern_XiaoQiDui(parseResult))
            {
                return true;
            }
            return false;
        ...
        {
            if(!BBMJMahjongAlgorithm1.CheckIsJiangCard(parseResult.normalParse.pair))
            {
                return false;
            }
            ...
            for(var i=0;i<parseResult.normalParse.triplyAry.length;i++)
            {
                ...
                if(!BBMJMahjongAlgorithm1.CheckIsJiangCard(parseResult.normalParse.triplyAry[i].cTokenCard))
                {
                    return false;
                }
            }
        ...
        {
            if(parseResult.is7Pair && (BBMJMahjongDef.gCardCount_Active == parseResult.cardAryBy7Pair.length))
            {
                ...
                BBMJMahjongAlgorithm1.sortCardAry(handCard);
        ...
        {
            //同时满足清一色,对对胡
            if (BBMJMahjongPattern.CheckMahjongPattern_QingYiShe(parseResult) && BBMJMahjongPattern.CheckMahjongPattern_DuiDuiHu(parseResult))
            {
                return true;
            }
            return false;
        ...
        //         return false;
        //     }
        //     if(!BBMJMahjongAlgorithm1.CheckIs19Card(parseResult.normalParse.pair))
        //     {
        //         return false;
        //     }
        ...
        //             {
        //                 if((7 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (8 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (9 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (1 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (2 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) ||
        //                     (3 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)))
        //                 {
        //                     bChangeCheck=true;
        //                     continue;
        //                 }
        ...
        //             if(enTripleType.TripleType_Echo == parseResult.normalParse.triplyAry[i].enTripleType)
        //             {
        //                 if((1 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (2 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) || 
        //                     (3 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)))
        //                 {
        //                     b123[BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard) - 1] = true;
        ...
        //                     (9 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)))
        //                 {
        //                     b789[BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard) - 7] = true;
        ...
        //                         (3 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard))))
        //                     {
        //                         if((BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[0]) != BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[1])) || 
        //                             (BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[0]) != BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[2])) ||
        //                             (BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[1]) != BBMJMahjongAlgorithm1.GetMahjongColor(b123Triple[2])))
        //                         {
        //                             return false;
        //                         }
        //                         else
        ...
        //                         ((7 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) ||
        //                         (8 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard)) ||
        //                         (9 == BBMJMahjongAlgorithm1.GetMahjongValue(parseResult.normalParse.triplyAry[i].cTokenCard))))
        //                     {
        //                         if((BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[0]) != BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[1])) ||
        //                             (BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[0]) != BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[2])) ||
        //                             (BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[1]) != BBMJMahjongAlgorithm1.GetMahjongColor(b789Triple[2])))
        //                         {
        //                             return false;
        //                         }
        //                         else
        ...
        private static CheckMahjongPattern_QingYiShe(parseResult:clsParseResult):boolean
        {
            ...
            if (parseResult.is7Pair)
            {
                for(var i=0;i<parseResult.cardAryBy7Pair.length;i++)
                {
                    color[BBMJMahjongAlgorithm1.GetMahjongColor(parseResult.cardAryBy7Pair[i])] = 1;
                }
            }
            else
            {
                ...
                {
                    color[BBMJMahjongAlgorithm1.GetMahjongColor(parseResult.normalParse.activeCard[i])] = 1;
                }
            }
        ...
        public static Parse3ModelToTripleList(vSingleCard:Array<number>, vTriple:Array<clsParseTriple>):void 
        {
            ...
            //2、进行一次排序
            BBMJMahjongAlgorithm1.sortCardAry(vSingleCard);
                ...
                if ((vSingleCard.length % 3) == 0)
                {
                    for (var i = 0; i < vSingleCard.length - 2; i++)
                    {
                        ...
                        if ((vSingleCard[i] == vSingleCard[i + 1]) && (vSingleCard[i] == vSingleCard[i + 2]))
                        {
                            ...
                            BBMJMahjongPattern.Parse3ModelToTripleList(vRemCard, vSubTriple);
        ...
        public static ParseSingleColorCard(vSingleCard:Array<number>, pairTripleWrapper:clsPairTripleWrapper):void 
        {
            ...
            if (vSingleCard.length % 3 == 0)
            {
                ...
                BBMJMahjongPattern.Parse3ModelToTripleList(vSingleCard, vTriple);
            ...
            * ***********************************************************************************/
            ...
            for (var i = 0; i < vSingleCard.length - 1; i++)
            {
                ...
                if (vSingleCard[i] == vSingleCard[i + 1])
                {
                    ...
                    var vTriple:Array<clsParseTriple> = new Array<clsParseTriple>();
                    BBMJMahjongPattern.Parse3ModelToTripleList(vRemList, vTriple);
        ...
        public static IsHuNormalStruct(vSrc:Array<number>):boolean
        {
            ...
            for (var i = 0; i < vSrc.length - 1; i++)
            {
                if (vSrc[i] == vSrc[i + 1])//找到一对
                {
                    ...
                    //2、分拣牌
                    BBMJMahjongAlgorithm1.spiltHandCard(vLeftCard, vWanCard, vTongCard, vTiaoCard, vZhiCard);

                    //3、分别进行解析
                    BBMJMahjongPattern.Parse3ModelToTripleList(vWanCard, v_TripleWan);
                    BBMJMahjongPattern.Parse3ModelToTripleList(vTongCard, v_TripleTong);
                    BBMJMahjongPattern.Parse3ModelToTripleList(vTiaoCard, v_TripleTiao);
                    BBMJMahjongPattern.Parse3ModelToTripleList(vZhiCard, v_TripleZhi);
