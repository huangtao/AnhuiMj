import { LQMJSoundDef, LQMJ } from "./ConstDef/LQMJMahjongDef";
import { AudioType, VoiceType } from '../../CustomType/Enum';

const { ccclass, property } = cc._decorator;

@ccclass
export default class M_LQMJVoice{
    ...
    public static PlayChatVoice(value: number, gender: number,Type:VoiceType) {
        ...
        LQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }else{
        ...
        // var path = "resources/gameres/M_LQMJ/sound/";
        // if (gender == 1) {
    ...
    public static PlayCardType(str:string) {
        var path = LQMJSoundDef.VoicePath;
        // if (gender == 1) {
    ...
    public static PlayPublicSound(value: string) {
        var path = cc.url.raw(LQMJSoundDef.CommonVoicePath + "Voice/" + value + ".mp3");
        LQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放系统音效
    ...
    public static PlaySysSound(value: string) {
        var path = cc.url.raw(LQMJSoundDef.VoicePath + "SysSound/Sys76_" + value + ".mp3");
        LQMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放bgm
    ...
    public static PlayBgm() {
        ...
        LQMJ.ins.iclass.playMJSound(path, AudioType.Music, true);
    }
