...
import { IBBMJClass, BBMJTableConfig, BBMJTimer, enGamePhase, BBMJOutCardPlayer, BBMJMahjongDef, BBMJ, BBMJTimerDef, enHuCardType, enBBMJAniType, enFixedCardType } from "./ConstDef/BBMJMahjongDef";
import { GameIF } from "../../CommonSrc/GameIF";
import { QL_Common } from "../../CommonSrc/QL_Common";

import { AudioType } from "../../CustomType/Enum";
import M_BBMJVideoView from "./M_BBMJVideoView";
...
@ccclass
export default class M_BBMJVideoClass extends GameVideoBase implements IBBMJClass {
    ...
    private static _ins: M_BBMJVideoClass;
    public static get ins(): M_BBMJVideoClass { return this._ins; }
    ...
    @property(M_BBMJVideoView)
    private gameView: M_BBMJVideoView=null;
    //判断补花一次的音效
    ...
    * */
    ...
    protected OnPlayerSitDown(chairID: number, player: QL_Common.TablePlayer): void {
        ...
        if(chair == 0){
            M_BBMJVideoView.ins.ShowTimerView(this.ChairID);
        }
    }
    ...
    protected OnGameMessage(sendChair: number, msg: GameIF.CustomMessage): void {
        switch(msg.wSubCmdID){
                ...
                case M_BBMJ_GameMessage.BBMJMsgID_s2c.CMD_S_VoteQGResult:{
    ...
    private Handle_CMD_S_FanKaiHun(msg: GameIF.CustomMessage):void{
       ...
       //     M_BBMJVideoView.ins.CardView.hunPi.ShowCard(this._hunPiCard);
    ...
    private Handle_CMD_S_BuHua(msg: GameIF.CustomMessage): void {
        ...
        //M_BBMJVideoView.ins.CardView.hideOutCardArrow();
        ...
        M_BBMJVideoView.ins.CardView.addCard2Flower(playerBuHua.huacard, playerBuHua.chair);
        let sex:number=this.TablePlayer[playerBuHua.chair].Gender==1?1:2;

            if (this.oncebuhua){
                M_BBMJVoice.PlayCardType(`/sound/1/mj_buhua_${sex}.mp3`);
                this.oncebuhua=false;
        }



       // M_BBMJVoice.PlayCardType(`/sound/${sex}/mj_buhua_${sex}.mp3`);
        

        M_BBMJVideoView.ins.CardView.playerBuHua(playerBuHua.chair, playerBuHua.huacard);


        //如果是自己
         if (this.SelfChair == playerBuHua.chair/* && !isHuaCard*/) {
             BBMJMahjongAlgorithm1.delCard(this._handCard[this.SelfChair], [playerBuHua.huacard]);
             BBMJMahjongAlgorithm1.sortCardAry(this._handCard[this.SelfChair]);
             this._handCard[this.SelfChair]=BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[this.SelfChair],this.GetHunCardAry());

             //M_BBMJVideoView.ins.CardView.selfActive.refreshHandCardData(this._handCard[this.SelfChair]);
            M_BBMJVideoView.ins.CardView.selfActive.activeEnable(false);
           

            //this.stopTimer();
            // M_BBMJVideoView.ins.TimerView.hideArrow();
            // M_BBMJVideoView.ins.CardView.selfActive.allDown();
    ...
    private Handle_CMD_S_PlayerHoldCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        if(BBMJMahjongDef.gInvalidMahjongValue != playerHoldCard.card){
    ...
    private Handle_CMD_S_VoteRight(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        if(this._voteRight==BBMJMahjongDef.gVoteRightMask_Ting){
            ...
            this.scheduleOnce(function () {
             ...
             M_BBMJVideoView.ins.OperatorView.showOP(BBMJMahjongAlgorithm1.CheckVoteRight_Peng(this._voteRight),
    ...
    private Handle_CMD_S_PlayerOutCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //this.gameView.CardView.hideOutCardArrow();
        //牌阵数据处理
        BBMJMahjongAlgorithm1.delCard(this._handCard[playerOutCard.chair],[playerOutCard.card]);
        BBMJMahjongAlgorithm1.sortCardAry(this._handCard[playerOutCard.chair]);
         this._handCard[playerOutCard.chair]=BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[playerOutCard.chair],this.GetHunCardAry());
    }
    /**
     * 删除玩家牌池牌
     * */
    ...
    private Handle_CMD_S_PlayerPengCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //动画
        this.gameView.playBBMJAni(playerPeng.chair,enBBMJAniType.aniType_peng);
        this.gameView.CardView.hideOutCardArrow();
        //清理玩家打出的牌
        this._outCardPlayer.clear();
        //this.gameView.OutCardView.show=false;
        ...
        //牌阵数据处理
        BBMJMahjongAlgorithm1.delCard(this._handCard[playerPeng.chair],[playerPeng.card]);
        BBMJMahjongAlgorithm1.sortCardAry(this._handCard[playerPeng.chair]);
        this._handCard[playerPeng.chair]=BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[playerPeng.chair],this.GetHunCardAry());
    ...
    private Handle_CMD_S_PlayerAnGangCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //动画
        this.gameView.playBBMJAni(playerAGang.chair,enBBMJAniType.aniType_anGang);
        ...
        //牌阵数据处理
        BBMJMahjongAlgorithm1.delCard(this._handCard[playerAGang.chair],[playerAGang.card,playerAGang.card,playerAGang.card,playerAGang.card]);
        BBMJMahjongAlgorithm1.sortCardAry(this._handCard[playerAGang.chair]);
        this._handCard[playerAGang.chair]=BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[playerAGang.chair],this.GetHunCardAry());
    ...
    private Handle_CMD_S_PlayerMingGang(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //动画
        this.gameView.playBBMJAni(playerMGang.chair,enBBMJAniType.aniType_minggGang);
        this.gameView.CardView.hideOutCardArrow();
        this._outCardPlayer.clear();
        //this.gameView.OutCardView.show = false;
        ...
        //牌阵数据处理
        BBMJMahjongAlgorithm1.delCard(this._handCard[playerMGang.chair],[playerMGang.card,playerMGang.card,playerMGang.card]);
        BBMJMahjongAlgorithm1.sortCardAry(this._handCard[playerMGang.chair]);
        this._handCard[playerMGang.chair]=BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[playerMGang.chair],this.GetHunCardAry());
    }
    /**
     * 玩家补杠/巴杠
     * */
    private Handle_CMD_S_PlayerBuGangCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //动画
        this.gameView.playBBMJAni(playerBGang.chair,enBBMJAniType.aniType_minggGang);
        
        //处理补杠牌
        this.gameView.CardView.playerBGang(playerBGang.chair,playerBGang.card);

        //牌阵数据处理
        BBMJMahjongAlgorithm1.delCard(this._handCard[playerBGang.chair],[playerBGang.card]);
        BBMJMahjongAlgorithm1.sortCardAry(this._handCard[playerBGang.chair]);
        this._handCard[playerBGang.chair]= BBMJMahjongAlgorithm1.sortCardAry1(this._handCard[playerBGang.chair],this.GetHunCardAry());
    }
    ...
    private Handle_CMD_S_PlayerHuCard(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //this.PlaySound(BBMJSoundDef.sound_hu);
        switch(playerHu.huType){
            ...
            case enHuCardType.HuCardType_PingHu:{
                ...
                //this.gameView.OutCardView.show = false;
                this.gameView.playBBMJAni(playerHu.chair,enBBMJAniType.aniType_huCard);
                break;
            }
            ...
            case enHuCardType.HuCardType_ZiMo:{
                M_BBMJVoice.PlayCardType(`/sound/1/hu_${sex}.mp3`);
                this.gameView.playBBMJAni(playerHu.chair,enBBMJAniType.aniType_ziMo);
                break;
            }
    ...
    private Handle_CMD_S_DelQiangGangCard(sendChair : number,msg: GameIF.CustomMessage):void{
        ...
        BBMJMahjongAlgorithm1.delCard(this._handCard[sendChair],[qiangGangCard.card]);
    }
    /**
     * 某个玩家手中牌
     * */
    ...
    private Handle_CMD_S_GameFlow(sendChair : number,msg: GameIF.CustomMessage): void {
        var gameFlow: M_BBMJ_GameMessage.CMD_S_GameFlow = <M_BBMJ_GameMessage.CMD_S_GameFlow>msg;
    ...
    private Handle_CMD_S_TableCreatorInfo(sendChair : number,msg: GameIF.CustomMessage): void {
        ...
        //this.dispatchEvent(new BBMJEvent(BBMJEvent.msg_tableCreatorInfo,this._tableConfig.tableCreatorChair,this._tableConfig.tableCreatorID));
    }
    ...
    public regTimer(timerid:number,timerLen:number,chair:number):void{
        ...
        M_BBMJVideoView.ins.TimerView.node.active=true;
        M_BBMJVideoView.ins.TimerView.showArrow = chair;
        M_BBMJVideoView.ins.TimerView.timerNum = timerLen;
    ...
    private handleOPAfter():void{
          M_BBMJVideoView.ins.OperatorView.node.active=false;
          M_BBMJVideoView.ins.SelGangView.node.active=false;
        this._gamePhase = enGamePhase.GamePhase_Unknown;
    }
    ...
    private Handle_CMD_S_Ting(msg: GameIF.CustomMessage): void {
         ...
         M_BBMJVideoView.ins.GameStatusUserInfo.Ting=playerTing.TingNum;

   

        // for (var i = 0; i < M_BBMJVideoView.ins.CardView.getFixed(playerTing.TingNum)._fixedData.length; i++) {
        //     if (M_BBMJVideoView.ins.CardView.getFixed(playerTing.TingNum)._fixedData[i].fixedType == enFixedCardType.FixedCardType_AGang) {
              
        //         M_BBMJVideoView.ins.CardView.getFixed(playerTing.TingNum)._fixedData[i].bmp_cardcolorAry[3].node.active = false;
        //     }
        // }
    }
