...
import HQMJ_SinglePlayerFX from "./HQMJ_SinglePlayerFX";
...
@ccclass
export default class HQMJ_FenXiang extends cc.Component {
    ...
    //单个玩家分享信息
    @property([HQMJ_SinglePlayerFX])
    playAry: HQMJ_SinglePlayerFX[]=[];
  ...
  //明杠次数
   ...
   img_head:cc.Sprite[] = [];
    ...
    public init() {
        ...
        // for (var n: number = 0; n < HQMJMahjongDef.gPlayerNum; n++) {
        //     if(0 == n){
    ...
    //         M_HQMJView.ins.GameJiFenBan.node.active = true;
    //     }, 0.3);  
    // }
    ...
    public share(){
        ...
        this.btn_share.scheduleOnce(p=>{
            M_HQMJClass.ins.ScreenCapture(true,this.node);
        }, 0.3);   
    }
    ...
    public copyScore(){
        ...
        M_HQMJClass.ins.CopyToClipboard(scoreStr);
    }
    ...
    public startShow(fun: () => void = null, obj: any = null): void {
        ...
        for (let i = 0; i < HQMJMahjongDef.gPlayerNum; i++) {
            ...
            if(null != M_HQMJClass.ins.TablePlayer[i])
                LoadHeader(M_HQMJClass.ins.TablePlayer[i].FaceID, this.img_head[i]);
        }
        M_HQMJVoice.PlayCardType(`/sound/final_report.mp3`);
        this.onShow();
    }
    ...
    protected onShow(): void {
        //播放游戏结束音效
       // M_HQMJVoice.GameOver();
        this.node.active = true;
        this.lab_other.string="霍邱麻将";
        //游戏规则
        var rule = M_HQMJClass.ins.GameRule;
        var rule0 = "";
        var rule1 = "";
        ...
        this.HQMJClass.sendData(queryRecord);
    ...
    private loadRecordData(): void {
        let time = new Date();
        if(M_HQMJClass.ins.TableConfig.tableWhere > 0){//亲友圈
            this.lal_club.node.active = true;
            this.lal_clubNum.node.active = true;
            this.lal_clubNum.string = M_HQMJClass.ins.TableConfig.tableWhere.toString();
            this.lal_room.string="亲友圈房号:"+this.HQMJClass.getTableConfig().TableCode;
        }else{
            this.lal_club.node.active = false;
            this.lal_clubNum.node.active = false;
            this.lal_room.string="房间号:"+this.HQMJClass.getTableConfig().TableCode;
        }
        ...
        this.lal_jushu.string=this.HQMJClass.getTableConfig().alreadyGameNum.toString();
