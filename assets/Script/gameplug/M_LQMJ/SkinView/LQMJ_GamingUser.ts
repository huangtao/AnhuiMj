...
@ccclass
export default class LQMJ_GamingUser extends cc.Component {
    ...
    public SetPlayer(player: QL_Common.TablePlayer):void{   
        ...
        let ss="";
        let le=LQMJMahjongAlgorithm.strLen(player.NickName)
        if( le > 4){
    ...
    public SetScore(score:number):void{
        this.userScore.showUserScore(score,LQMJ.ins.iclass.getTableConfig().isValid);
    }

    public AddScore(score:number):void{
        this.userScore.addUserScore(score,LQMJ.ins.iclass.getTableConfig().isValid);
    }
    ...
    public SetLa(score:number):void{
        ...
        let url="";
        if(score>=0 && score<=2){
            url=`gameres/M_LQMJ/Texture/LaPaoZuo/hong${score}`;
            console.log(url);
    ...
    public SetPao(score:number):void{
        let url="";
        if(score>=0 && score<=2){
            url=`gameres/M_LQMJ/Texture/LaPaoZuo/huan${score}`;
