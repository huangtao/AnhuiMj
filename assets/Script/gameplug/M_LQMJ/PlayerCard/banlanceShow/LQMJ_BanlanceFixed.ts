...
import LQMJ_SingleFixed from "../single/banlanceShow/LQMJ_SingleFixed";
...
@ccclass
export default class LQMJ_BanlanceFixed extends LQMJ_CardBase {
    ...
    //定牌数据
    protected _fixedData:Array<LQMJ_SingleFixed>;
    
    public init() {
        if(this._fixedData==null)
        this._fixedData = new Array<LQMJ_SingleFixed>(0);
    }
    ...
    public addFixed(card:number,fixedType:enFixedCardType,pos:number,chiType:number):void{
        ...
        let newnode = LQMJ_BanlanceFixed._freeNode.get();
        if (!cc.isValid(newnode)) {
            newnode = cc.instantiate(this.FixedType);
        }
        var fixed = newnode.getComponent<LQMJ_SingleFixed>(LQMJ_SingleFixed);
        fixed.init();
        this.node.addChild(newnode);
        this._fixedData.push(fixed);
        fixed.showCard(card,fixedType,chiType,pos,LQMJ.ins.iclass);
    ...
    public clear(): void {
        ...
        while (this.node.children.length > 0) {
            ...
            if (cc.isValid(fixed)) {
                ...
                //     fixed.destroy();
                // } else {
                    LQMJ_BanlanceFixed._freeNode.put(fixed);
                // }
            } else {
                ReportError("手牌数据出错！！！");
            }
