...
@ccclass
export default class HZMJ_SelfSingleActive extends HZMJ_SingleActiveBase {
    ...
    private onTouchBegin(e:cc.Event.EventTouch):void{
        //cc.log("1111111");
        if(M_HZMJClass.ins.ifCanOp && M_HZMJClass.ins.IfCanOutACard(this._cardValue) && cc.isValid(e)){
    ...
    private onTouchEnd(e:cc.Event.EventTouch):void{
        ...
        if(this._isTouch){
            ...
            if(tempY >=30 ) {
                ...
                if(M_HZMJClass.ins.ifCanOp)
                {   
                    //cc.log("33333");
                    this.node.opacity=0;
                    this.down();
                    this._isValid=false;
                    this.node.dispatchEvent(new HZMJEvent(HZMJEvent.msg_outACard,this._cardValue));
                    
                    //this.node.destroy();
                }
                else{
                    cc.log("44444");
                    this.node.dispatchEvent(new HZMJEvent(HZMJEvent.msg_reflashHandCard));
                }
            }
    ...
    private onClick():void{
        ...
        //如果已经胡牌
        if(HZMJ.ins.iclass.isAlreadyHu()){
            return;
        }
        cc.log("点击了！"+this._cardValue);
        if(HZMJMahjongDef.gInvalidMahjongValue != this._cardValue){
            
                if(M_HZMJClass.ins.ifCanOp && M_HZMJClass.ins.IfCanOutACard(this._cardValue)){
                    ...
                    if(this._isUp){//如果已经站立,打出去
                        ...
                        M_HZMJClass.ins.showHideCard(this.cardValue);
                        if(this._ifCanTingAfterOutThisCard) {
                            //显示听牌
                            M_HZMJClass.ins.showTingCard(this.cardValue,this.node.x);
                        }else{
    ...
    public showBackCard():void{
        ...
        // this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        //this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjong3DPaiBeiRes("b_btm_self_mj_bg");
        if(HZMJ.ins.iclass.is2D()){
            this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        }
        else{
            this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjong3DPaiBeiRes("chi_self_2_13");
        }
        ...
        if(HZMJ.ins.iclass.is2D()){
            ...
            if(true){
                ...
                if(HZMJMahjongDef.gBackMahjongValue != card){
                    ...
                    this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_showcard_self_1280");
                ...
                //url=`gameres/gameCommonRes/Texture/Mahjong/PaiBei3/pb3_active_self_1280`;
                //url1=HZMJ.ins.iclass.getMahjongResName(card);
                //this.bmp_cardback.spriteFrame=HZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_active_self_1280");
               // this.bmp_cardcolor.spriteFrame=HZMJ.ins.iclass.getMahjongPaiHuaRes(card);
                //SetTextureResAry([url,url1],[this.bmp_cardback,this.bmp_cardcolor]);
