...
import LQMJ_SinglePlayerRecord from "./LQMJ_SinglePlayerRecord";
import M_LQMJClass from "../M_LQMJClass";
import LQMJ_HeadPortrait from "./LQMJ_HeadPortrait";

const { ccclass, property } = cc._decorator;

@ccclass
export default class LQMJ_JiFenBan extends cc.Component {
    @property(cc.Button)
    ...
    private _touxiangAry:Array<LQMJ_HeadPortrait>;
    ...
    //记录数据
    private _gameRecordAry:Array<M_LQMJ_GameMessage.GameRecordResult>;
    //是否取过数据
    ...
    public constructor() {
        ...
        this._gameRecordAry = new Array<M_LQMJ_GameMessage.GameRecordResult>();
    ...
    private InitTouXiang():void{
        this._touxiangAry=new Array<LQMJ_HeadPortrait>();
        for(var i=0;i<this.pfb_touXiang.length;i++){
            var toux=cc.instantiate(this.pfb_touXiang[i]);
            this._touxiangAry.push(toux.getComponent<LQMJ_HeadPortrait>(LQMJ_HeadPortrait));
    ...
    onLoad() {
        ...
        public gameRecord(recordData:Array<number>):void{
            ...
            var gameRecord: M_LQMJ_GameMessage.GameRecordResult = new M_LQMJ_GameMessage.GameRecordResult();
         ...
         * 游戏记录数据到来
         * */
        public gameRecordDataCome(recordData: Array<M_LQMJ_GameMessage.GameRecordResult>):void{
        ...
        protected onShow(): void {
            ...
            this.lbl_gamenum.string="局号："+LQMJ.ins.iclass.getTableConfig().TableCode;
            for(var i:number=0; i<LQMJMahjongDef.gPlayerNum; i++){


                //this._img_accountAry[i].source=LQMJ.ins.iclass.getTablePlayerAry()[i].FaceID;

                this._touxiangAry[i].setHeadPortSpri(LQMJ.ins.iclass.getTablePlayerAry()[i].FaceID);

                let name:string=LQMJ.ins.iclass.getTablePlayerAry()[i].NickName;
                if (name.length > 4){
                ...
                if (i==LQMJ.ins.iclass.getSelfChair()) {
            ...
            //if(!this._hasReadData){
            var queryRecord :M_LQMJ_GameMessage.CMD_C_QueryGameRecord = new M_LQMJ_GameMessage.CMD_C_QueryGameRecord();
            queryRecord.queryNum = 0;
            LQMJ.ins.iclass.sendData(queryRecord);
            this.loadRecordData();
            // }else{
            //     this.loadRecordData();
            // }
        ...
        private loadRecordData():void{
            ...
            for(var i: number = 0;i < this._gameRecordAry.length;i++) {
                // var record: LQMJ_SinglePlayerRecord = new LQMJ_SinglePlayerRecord(this._gameRecordAry[i],i);
                // this._recordListContainer.addChild(record);
                // record.node.y = i * 61;
            //       let newnode = LQMJ_JiFenBan._freeNode.get();
            // if (!cc.isValid(newnode)) {
            //     newnode = cc.instantiate(this.single);
            // }
            //   let sin=newnode.getComponent<LQMJ_SinglePlayerRecord>(LQMJ_SinglePlayerRecord);
            ...
            // this.group.addChild(newnode);
                ...
                for(var j:number=0; j<LQMJMahjongDef.gPlayerNum; j++){
        ...
        private AddDataToContain(data:M_LQMJ_GameMessage.GameRecordResult,i:number){
         //   var dt=cc.instantiate(this.pfb_SinglePlayerRecord);
          ...
          //  sin.init(this._gameRecordAry[i],i);
            ...
            newnode.y=i*61;
            var xx:LQMJ_SinglePlayerRecord=newnode.getComponent<LQMJ_SinglePlayerRecord>(LQMJ_SinglePlayerRecord);
            xx.Init(data,i);
