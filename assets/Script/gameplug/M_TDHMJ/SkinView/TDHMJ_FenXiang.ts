...
import TDHMJ_SinglePlayerFX from "./TDHMJ_SinglePlayerFX";
...
@ccclass
export default class TDHMJ_FenXiang extends cc.Component {
    ...
    //单个玩家分享信息
    @property([TDHMJ_SinglePlayerFX])
    playAry: TDHMJ_SinglePlayerFX[]=[];
  ...
  //明杠次数
   ...
   img_head:cc.Sprite[] = [];
    ...
    public init() {
        ...
        // for (var n: number = 0; n < TDHMJMahjongDef.gPlayerNum; n++) {
        //     if(0 == n){
    ...
    //         M_TDHMJView.ins.GameJiFenBan.node.active = true;
    //     }, 0.3);  
    // }
    ...
    public share(){
        ...
        this.btn_share.scheduleOnce(p=>{
            M_TDHMJClass.ins.ScreenCapture(true,this.node);
        }, 0.3);   
    }
    ...
    public copyScore(){
        ...
        M_TDHMJClass.ins.CopyToClipboard(scoreStr);
    }
    ...
    public startShow(fun: () => void = null, obj: any = null): void {
        ...
        for (let i = 0; i < TDHMJMahjongDef.gPlayerNum; i++) {
            ...
            if(null != M_TDHMJClass.ins.TablePlayer[i])
                LoadHeader(M_TDHMJClass.ins.TablePlayer[i].FaceID, this.img_head[i]);
        }
        M_TDHMJVoice.PlayCardType(`/sound/final_report.mp3`);
        this.onShow();
    }
    ...
    protected onShow(): void {
        //播放游戏结束音效
       // M_TDHMJVoice.GameOver();
        this.node.active = true;
        this.lab_other.string="推倒胡";
        //游戏规则
        var rule = M_TDHMJClass.ins.GameRule;
        var rule0 = "";
        var rule1 = "";
        ...
        this.TDHMJClass.sendData(queryRecord);
    ...
    private loadRecordData(): void {
        let time = new Date();
        if(M_TDHMJClass.ins.TableConfig.tableWhere > 0){//亲友圈
            this.lal_club.node.active = true;
            this.lal_clubNum.node.active = true;
            this.lal_clubNum.string = M_TDHMJClass.ins.TableConfig.tableWhere.toString();
            this.lal_room.string="亲友圈房号:"+this.TDHMJClass.getTableConfig().TableCode;
        }else{
            this.lal_club.node.active = false;
            this.lal_clubNum.node.active = false;
            this.lal_room.string="房间号:"+this.TDHMJClass.getTableConfig().TableCode;
        }
        ...
        this.lal_jushu.string=this.TDHMJClass.getTableConfig().alreadyGameNum.toString();
