...
export default class BBMJMahjongAlgorithmHaveHunLhh {
    ...
    public static GetTingCardArray(srcAry:Array<number>,HunAry:Array<number>,flowerAry:Array<number>=null):Array<number>{
            ...
            {
                if(flowerAry!=null){
                    if(BBMJMahjongAlgorithm1.IsContainsNumber(flowerAry,BBMJMahjongDef.gMahjongCard[i])){//花牌不提示在胡牌列表里面
                        continue;
                    }
                }
                ...
                if (BBMJMahjongAlgorithmHaveHunLhh.CheckIfCanHuCardArrayForRQMJ(allCard,HunAry))
        ...
        public static CheckIfCanHuCardArrayForRQMJ(srcAry:Array<number>,HunAry:Array<number>):boolean{
            ...
            */
            ...
            //    return true;
            //}
            // if (BBMJMahjongAlgorithmHaveHunLhh.IsHuNormalStructHaveHun(vSrc,HunAry))
            // {
            //     return true;
            // }
            return false;
        ...
        private static IsHuNormalStructHaveHun(vSrc:Array<number>,HunAry:Array<number>):boolean{
            ...
            {
                if (BBMJMahjongAlgorithm1.IsContainsNumber(HunAry,vSrc[i]))
                    laiziNum++;
                else
                {
                    srcAry.push(vSrc[i]);
            ...
            //宿州麻将是否可以胡7对是一个可选项
            // if(BBMJMahjongAlgorithmHaveHunLhh.IsCanHu7Dui){
            //     //为任丘麻将加的
            ...
            //分情况
            BBMJMahjongAlgorithm1.sortCardAry(srcAry);
            //1、按花色分拣牌
            ...
            //3、得到各自的成组需赖子数
            var wanneed = BBMJMahjongAlgorithm1.ChangeArrayToTripleForHuapai(vectorWan);
            var tongneed = BBMJMahjongAlgorithm1.ChangeArrayToTripleForHuapai(vectorTong);
            var tiaoneed = BBMJMahjongAlgorithm1.ChangeArrayToTripleForHuapai(vectorTiao);
            var zineed = BBMJMahjongAlgorithm1.ChangeArrayToTripleForZipai(vectorZi);//红中赖子麻将实际上也没有字牌
            ...
            if (needNum <= laiziNum)
            {
                if (BBMJMahjongAlgorithm1.ChangeArrayToPairAndTripleForHuapai(vectorWan, laiziNum - needNum))
                    return true;
            }
            ...
            if (needNum <= laiziNum)
            {
                if (BBMJMahjongAlgorithm1.ChangeArrayToPairAndTripleForHuapai(vectorTong, laiziNum - needNum))
                    return true;
            }
            ...
            if (needNum <= laiziNum)
            {
                if (BBMJMahjongAlgorithm1.ChangeArrayToPairAndTripleForHuapai(vectorTiao, laiziNum - needNum))
                    return true;
            }
            needNum = tongneed + tiaoneed + wanneed;
            if(needNum<=laiziNum)
            {
                if(BBMJMahjongAlgorithm1.ChangeArrayToPairAndTripleForZipai(vectorZi,laiziNum-needNum))
                return  true;
            }
        ...
        public static ChangeArrayTo7PairsNeedHun(vectorCard:Array<number>,hunNum:number):boolean{
            ...
            if(BBMJMahjongAlgorithm1.GetCardTypeNumFromCardAry(vectorCard)>7)//牌类型数大于7不可能组成7对
            {
                return false;
            }
            if(vectorCard==null)
            ...
            needHun = BBMJMahjongAlgorithm1.ChangeArrayToPairsNeedHun(vectorCard);
       ...
       /////////////////////////////////////////////////////////////////////上面是获取出去某张牌后的听牌列表，下面是获取出哪些牌可以听牌//////////////////////////////////////////////////////////
        ...
        public static GetLastCardToTing(srcAry:Array<number>,HunAry:Array<number>):Array<number>{       
            ...
            for(var i:number=0;i<srcAry.length;i++)
            {
                ...
                {
                    //if(!BBMJMahjongAlgorithm1.IsContainsNumber(HunAry,srcAry[i]))//这句是红中麻将中赖子牌不加入提示，因为赖子牌不能出，而赖子牌能出的宿州麻将则需要把这个判断去掉
