...
@ccclass
export default class LQMJ_SingleFixedBase extends LQMJ_SingleCardBase {
     ...
     * */
    public showCard(card: number,fixedType: enFixedCardType,index:number,type:number,pos?:number,_lqmj = LQMJ.ins.iclass): void {
    ...
    public changePeng2Gang(card: number,_lqmj): boolean {
        if((enFixedCardType.FixedCardType_Peng == this._fixdType) && (card == this._cardValue)) {
            ...
            // }
            this.showCard(card,enFixedCardType.FixedCardType_BGang,this._cardIndex,this._pos,null,_lqmj);
