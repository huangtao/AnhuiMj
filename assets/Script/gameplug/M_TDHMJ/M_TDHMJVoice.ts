import { TDHMJSoundDef, TDHMJ } from "./ConstDef/TDHMJMahjongDef";
import { AudioType, VoiceType } from '../../CustomType/Enum';

const { ccclass, property } = cc._decorator;

@ccclass
export default class M_TDHMJVoice{
    ...
    public static PlayChatVoice(value: number, gender: number,Type:VoiceType) {
        ...
        TDHMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }else{
        ...
        // var path = "resources/gameres/M_TDHMJ/sound/";
        // if (gender == 1) {
    ...
    public static PlayCardType(str:string) {
        var path = TDHMJSoundDef.VoicePath;
        // if (gender == 1) {
    ...
    public static PlayPublicSound(value: string) {
        var path = cc.url.raw(TDHMJSoundDef.CommonVoicePath + "Voice/" + value + ".mp3");
        TDHMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放系统音效
    ...
    public static PlaySysSound(value: string) {
        var path = cc.url.raw(TDHMJSoundDef.VoicePath + "SysSound/Sys76_" + value + ".mp3");
        TDHMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放bgm
    ...
    public static PlayBgm() {
        ...
        TDHMJ.ins.iclass.playMJSound(path, AudioType.Music, true);
    }
