import { TingCardTip } from "../ConstDef/HQMJMahjongDef";
import M_HQMJClass from "../M_HQMJClass";
import HQMJ_SingleTingCardTip from "./HQMJ_SingleTingCardTip";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HQMJ_TingTip extends cc.Component {
    ...
    @property(cc.Prefab)
    HQMJ_SingleTingCard_Tip: cc.Prefab=null;
    ...
    //结算
    private _singleTingCard: HQMJ_SingleTingCardTip;
    /**
     * 结算视图
     * */
    public get SingleTingCard(): HQMJ_SingleTingCardTip {
    ...
    //听牌提示
    private _tingCardAry: Array<HQMJ_SingleTingCardTip>;
        ...
        if(this._tingCardAry!=null)
        {
            while (this._tingCardAry.length > 0) {
               // this._tingCardAry.pop().node.destroy();
                HQMJ_TingTip._freeNode.put(this._tingCardAry.pop().node);
            }
        }else{
            this._tingCardAry = new Array<HQMJ_SingleTingCardTip>();
        }
    ...
    public showTingTip(tingTip: Array<TingCardTip>,tips:boolean): void {
       ...
       // M_HQMJView.ins.TingTip.node.x=-330;
        ...
        for (var i: number = 0; i < tingTip.length; i++) {
          let singleTip = HQMJ_TingTip._freeNode.get();
            if (!cc.isValid(singleTip)) {
                singleTip = cc.instantiate(this.HQMJ_SingleTingCard_Tip);
            }
            this._singleTingCard=singleTip.getComponent<HQMJ_SingleTingCardTip>(HQMJ_SingleTingCardTip);
