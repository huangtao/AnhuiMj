...
@ccclass
export default class TDHMJ_OppoVideoActive extends TDHMJ_VideoActiveBase {
    ...
    //     //起始位置
    //     var startPos: number = TDHMJ_OppoVideoActive.ArrangeStartPos_lie[this.fixedCardNum];
    ...
    protected refreshHandCard(): void {
        if(M_TDHMJVideoClass.ins.is2D()){
            //起始位置
            let startPos: number = TDHMJ_OppoVideoActive.ArrangeStartPos_lie[this.fixedCardNum];

            //开始排版
            for(var i: number = 0;i < this._cardData.length;i++) {
                this._cardData[i].node.x = startPos - i * 37-640-18;
                this._cardData[i].node.y = 284;
                this._cardData[i].showCard(this._handCard[i],true,0,M_TDHMJVideoClass.ins);
            ...
            //起始位置
            let startPos: number = TDHMJ_OppoVideoActive.ArrangeStartPos3D_lie[this.fixedCardNum];
            this.resetZ();
            ...
            for(let i: number = 0;i < this._cardData.length;i++) {
                ...
                this._cardData[i].showCard(this._handCard[i],true,i+temp+1,M_TDHMJVideoClass.ins);
