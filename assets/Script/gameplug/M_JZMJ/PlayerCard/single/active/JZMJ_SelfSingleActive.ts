...
@ccclass
export default class JZMJ_SelfSingleActive extends JZMJ_SingleActiveBase {
    ...
    private onTouchBegin(e:cc.Event.EventTouch):void{
        //cc.log("1111111");
        if(M_JZMJClass.ins.ifCanOp && M_JZMJClass.ins.IfCanOutACard(this._cardValue) && cc.isValid(e)){
    ...
    private onTouchEnd(e:cc.Event.EventTouch):void{
        ...
        if(this._isTouch){
            ...
            if(tempY >=30 ) {
                ...
                if(M_JZMJClass.ins.ifCanOp)
                {   
                    //cc.log("33333");
                    this.node.opacity=0;
                    this.down();
                    this._isValid=false;
                    this.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_outACard,this._cardValue));
                    
                    //this.node.destroy();
                }
                else{
                    cc.log("44444");
                    this.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_reflashHandCard));
                }
            }
    ...
    public onClick():void{
        ...
        //如果已经胡牌
        if(JZMJ.ins.iclass.isAlreadyHu()){
            return;
        }
        cc.log("点击了！"+this._cardValue);
        if(JZMJMahjongDef.gInvalidMahjongValue != this._cardValue){
            
                if(M_JZMJClass.ins.ifCanOp && M_JZMJClass.ins.IfCanOutACard(this._cardValue)){
                    ...
                    if(this._isUp){//如果已经站立,打出去
                        ...
                        M_JZMJClass.ins.showHideCard(this.cardValue);
                        if(this._ifCanTingAfterOutThisCard) {
                            //显示听牌
                            M_JZMJClass.ins.showTingCard(this.cardValue,this.node.x,false);
                        }else{

                            //注释
                            // let index =0;
                            // if(!M_JZMJClass.ins._tingPai[M_JZMJClass.ins.SelfChair] && index == 0){
    ...
    public showBackCard():void{
        ...
        // this.bmp_cardback.spriteFrame=JZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        //this.bmp_cardback.spriteFrame=JZMJ.ins.iclass.getMahjong3DPaiBeiRes("b_btm_self_mj_bg");
        if(JZMJ.ins.iclass.is2D()){
            this.bmp_cardback.spriteFrame=JZMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        }
        else{
            this.bmp_cardback.spriteFrame=JZMJ.ins.iclass.getMahjong3DPaiBeiRes("chi_self_2_13");
        }
        ...
        if(_JZMJ.is2D()){
            ...
            if(true){
                ...
                if(JZMJMahjongDef.gBackMahjongValue != card){
                    ...
                    this.bmp_cardback.spriteFrame=_JZMJ.getMahjongPaiBeiRes("pb3_showcard_self_1280");
                    ...
                    this.bmp_cardback.spriteFrame=_JZMJ.getMahjongPaiBeiRes("pb3_backcard_self_1280");
                    //SetTextureRes(url,this.bmp_cardback);
                ...
                //url=`gameres/gameCommonRes/Texture/Mahjong/PaiBei3/pb3_active_self_1280`;
                //url1=_JZMJ.getMahjongResName(card);
                //this.bmp_cardback.spriteFrame=_JZMJ.getMahjongPaiBeiRes("pb3_active_self_1280");
               // this.bmp_cardcolor.spriteFrame=_JZMJ.getMahjongPaiHuaRes(card);
                //SetTextureResAry([url,url1],[this.bmp_cardback,this.bmp_cardcolor]);
            ...
            if(isLie){
                ...
                if(JZMJMahjongDef.gBackMahjongValue != card){
                    ...
                    this.showSelfDaoPai(_JZMJ);                                 

                    this.bmp_cardcolor.node.active = true;
                    this.bmp_cardback.node.active = true;
                }else{
                    //url=`gameres/gameCommonRes/Texture/Mahjong/PaiBei3/pb3_backcard_self_1280`;
                    // this.bmp_cardback.spriteFrame=_JZMJ.getMahjong3DPaiBeiRes("hand_self_1");
                    //SetTextureRes(url,this.bmp_cardback);
                    //this._bmp_cardback.texture = <egret.Texture>RES.getRes(switchResName("xzmj_backcard_self_png"));
                    this.bmp_enable.node.active=false;
                    this.showSelfDaoPai(_JZMJ);
    ...
    private showSelfDaoPai(_JZMJ):void{
