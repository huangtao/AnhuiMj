...
import HZMJ_SelfSingleActive from "../single/active/HZMJ_SelfSingleActive";
import HZMJ_SingleActiveBase from "../single/HZMJ_SingleActiveBase";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HZMJ_SelfActive extends HZMJ_ActiveBase {

    onLoad() {
        // init logic
        super.onLoad();
        this.init(0);
        this.node.on(HZMJEvent.HZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        for(var i: number = 0;i < holdNum;i++) {
            this._handCard.push(HZMJ.ins.iclass.getSelfHandCardData()[i]);//[holdIdx*4 + i]);
        }
            ...
            for(let i: number = 0;i < upCard.length;i++) {
                for(let j: number = 0;j < this._cardData.length;j++) {
                    if(upCard[i] == this._cardData[j].cardValue) {
                        (<HZMJ_SelfSingleActive>this._cardData[j]).setUp();
                        break;
                    }                      
                }
            }
    ...
    public setUpCardDown():void{
        for(let j: number = 0;j < this._cardData.length;j++) {
            (<HZMJ_SelfSingleActive>this._cardData[j]).setDown();
        }
    }
    /**
     * 设置听牌标志牌
    ...
    public showTingCardToken(cardAry: Array<number>): void {
        if((null == cardAry) || (cardAry.length < 1)){
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                (<HZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard = false;
            }
        ...
        for(var i:number=0; i<this._cardData.length; i++){
            for(var j:number=0; j<cardAry.length; j++){
                if(this._cardData[i].cardValue == cardAry[j]){
                    (<HZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard=true;
                    break;
                }
            }
        }
    ...
    public refreshCardStatus(): void {
            for(var j: number = 0;j < this._cardData.length;j++) {
                (<HZMJ_SelfSingleActive>this._cardData[j]).enable = true;
                (<HZMJ_SelfSingleActive>this._cardData[j]).showCoverBG = false;
            }
    }
    ...
    public arrangeHandCard(): void {
        ...
        // for(var i: number = 0;i < this._cardData.length;i++) {
        //     (<HZMJ_SelfSingleActive>this._cardData[i]).showBackCard();
        // }
    ...
    private arrangeCardComplete():void{
        ...
        // this.node.dispatchEvent(new HZMJEvent(HZMJEvent.msg_arrangeHandCardComplete)); 
    }
    ...
    public activeEnable(enable: boolean): void {
        for(var i:number=0; i<this._cardData.length; i++){
            (<HZMJ_SelfSingleActive>this._cardData[i]).enable =  enable;
            if(!enable){
                (<HZMJ_SelfSingleActive>this._cardData[i]).showCoverBG=false;
            }
        }
    }
    ...
    protected refreshHandCard(): void {
        ...
        if(HZMJ.ins.iclass.is2D()){
            //起始位置
            let startPos: number = HZMJ_SelfActive.ArrangeStartPos[this.fixedCardNum];
            //开始排版
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                ...
                if(this.isHoldAfter && (i == (this._cardData.length - 1))) {
                    ...
                    if(!this.isLie && !HZMJ.ins.iclass.isVideo())
                    {(<HZMJ_SelfSingleActive>this._cardData[i]).setUp();}
                }
            ...
            //起始位置
            let startPos: number = HZMJ_SelfActive.ArrangeStartPos3D[this.fixedCardNum];
            //开始排版
            for(let i: number = 0;i < this._cardData.length;i++) {
                //this._cardData[i].node.setLocalZOrder(i+1);
                this.resetZ();
                this._cardData[i].node.x = startPos + i * 76;
                this._cardData[i].node.y = -284;
    ...
    //             //newnode.on(HZMJEvent.HZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    //             //this.createSingleActiveCard(newnode);
    //             var active = newnode.getComponent<HZMJ_SingleActiveBase>(HZMJ_SingleActiveBase);
    //             active.init();
     ...
     * 单个活动牌事件
     * */
    private onSingleActiveEvent(e:HZMJEvent):void{
        
        switch(e.msgCode){
            case HZMJEvent.msg_reflashHandCard:{
                e.stopPropagation();
                ////cc.log("自己刷新！");
                this.refreshHandCard();
                break;
    ...
    protected createSingleActiveCard(p:cc.Node): void {
        //p.on(HZMJEvent.HZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    private OutCardAfter(outindex:number):void{
        //HZMJMahjongAlgorithm.delCard(this._handCard,[card]);
        // //cc.log("位序调整");
        // let str1="";
        ...
        // }
        // //cc.log(str1);
        let tempCard:HZMJ_SingleActiveBase;
