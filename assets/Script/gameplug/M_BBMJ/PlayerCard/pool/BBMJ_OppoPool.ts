import BBMJ_PoolBase from "./BBMJ_PoolBase";
import { BBMJ } from "../../ConstDef/BBMJMahjongDef";

const { ccclass, property } = cc._decorator;

@ccclass
export default class BBMJ_OppoPool extends BBMJ_PoolBase {
    ...
    private resetZ_2d(): void {
        for (let i: number = 0; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i+1);
            }
    }
    ...
    private resetZ(): void {
        if (this._poolCard.length <= 5) {
            for (let i: number = 0; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i);
            }
            ...
            for (let i: number = 5; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (this._poolCard.length - i + 5);
            }
        } else if (this._poolCard.length <= 15) {
            let x = this._poolCard.length - 9;
            for (let i: number = 9; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i - 9);
            }
            for (let i: number = 0; i < 5; i++) {
                this._poolCard[i].node.zIndex = (i + x);
            }
            for (let i: number = 5; i <  9; i++) {
                this._poolCard[i].node.zIndex = (9 - i + x);
            }

        } else if(this._poolCard.length <= 20){
            for (let i: number = 15; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (this._poolCard.length - i);
            }
            for (let i: number = 10; i < 15; i++) {
                this._poolCard[i].node.zIndex = (i - 10);
            }
            for (let i: number = 0; i < 5; i++) {
                this._poolCard[i].node.zIndex = (i + 10);
            }
            ...
            for (let i: number = 20; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i - 20);
            }
            for (let i: number = 15; i < 20; i++) {
                this._poolCard[i].node.zIndex = (29 - i);
            }
            for (let i: number = 10; i < 15; i++) {
                this._poolCard[i].node.zIndex = (i-5);
            }
            for (let i: number = 5; i < 10; i++) {
                this._poolCard[i].node.zIndex = (29-i);
            }
            for (let i: number = 0; i < 5; i++) {
                this._poolCard[i].node.zIndex = (15 + i);
            }
        }else if(this._poolCard.length <= 30){
            for (let i: number = 25; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (24-i);
            }
            for (let i: number = 20; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i - 20);
            }
    ...
    //         }
    //         for(let i: number = 5;i < 10;i++){
    //             this._poolCard[i].node.zIndex = (10-i+x);
    //         }
    ...
    //         }
    //         for(let i: number = 5;i < 10;i++){
    //             this._poolCard[i].node.zIndex = (20-i+x);
    //         }

    //         for(let i: number = 10;i < 15;i++){
    //             this._poolCard[i].node.zIndex = (i+x-10);
    //         }
    //         for(let i: number = 15;i < 20;i++){
    //             this._poolCard[i].node.zIndex = (20-i+x-10);
    //         }
