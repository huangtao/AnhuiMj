...
import JZMJ_SingleActive from "../single/banlanceShow/JZMJ_SingleActive";
import { ReportError } from "../../../../Tools/Function";

const { ccclass, property } = cc._decorator;

@ccclass
export default class JZMJ_BanlanceActive extends JZMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<JZMJ_SingleActive>;
    /**
     * 手牌
     * */
    ...
    public init() {
        this._cardData = new Array<JZMJ_SingleActive>(0);
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(var i: number = 0;i < newNum;i++) {
              let newnode = JZMJ_BanlanceActive._freeNode.get();
                ...
                var active = newnode.getComponent<JZMJ_SingleActive>(JZMJ_SingleActive);
                active.init();
    ...
    public clear(): void {
        ...
        while (this.node.children.length > 0) {
            ...
            if (cc.isValid(active)) {
                ...
                //     active.destroy();
                // } else {
                    JZMJ_BanlanceActive._freeNode.put(active);
                // }
            } else {
                ReportError("手牌数据出错！！！");
            }
