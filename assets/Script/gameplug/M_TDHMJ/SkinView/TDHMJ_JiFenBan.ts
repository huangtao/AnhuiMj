...
import TDHMJ_SinglePlayerRecord from "./TDHMJ_SinglePlayerRecord";
import M_TDHMJClass from "../M_TDHMJClass";
import TDHMJ_HeadPortrait from "./TDHMJ_HeadPortrait";

const { ccclass, property } = cc._decorator;

@ccclass
export default class TDHMJ_JiFenBan extends cc.Component {
    @property(cc.Button)
    ...
    private _touxiangAry:Array<TDHMJ_HeadPortrait>;
    ...
    //记录数据
    private _gameRecordAry:Array<M_TDHMJ_GameMessage.GameRecordResult>;
    //是否取过数据
    ...
    public constructor() {
        ...
        this._gameRecordAry = new Array<M_TDHMJ_GameMessage.GameRecordResult>();
    ...
    private InitTouXiang():void{
        this._touxiangAry=new Array<TDHMJ_HeadPortrait>();
        for(var i=0;i<this.pfb_touXiang.length;i++){
            var toux=cc.instantiate(this.pfb_touXiang[i]);
            this._touxiangAry.push(toux.getComponent<TDHMJ_HeadPortrait>(TDHMJ_HeadPortrait));
    ...
    onLoad() {
        ...
        public gameRecord(recordData:Array<number>):void{
            ...
            var gameRecord: M_TDHMJ_GameMessage.GameRecordResult = new M_TDHMJ_GameMessage.GameRecordResult();
         ...
         * 游戏记录数据到来
         * */
        public gameRecordDataCome(recordData: Array<M_TDHMJ_GameMessage.GameRecordResult>):void{
        ...
        protected onShow(): void {
            ...
            this.lbl_gamenum.string="局号："+TDHMJ.ins.iclass.getTableConfig().TableCode;
            for(var i:number=0; i<TDHMJMahjongDef.gPlayerNum; i++){


                //this._img_accountAry[i].source=TDHMJ.ins.iclass.getTablePlayerAry()[i].FaceID;

                this._touxiangAry[i].setHeadPortSpri(TDHMJ.ins.iclass.getTablePlayerAry()[i].FaceID);

                let name:string=TDHMJ.ins.iclass.getTablePlayerAry()[i].NickName;
                if (name.length > 4){
                ...
                if (i==TDHMJ.ins.iclass.getSelfChair()) {
            ...
            //if(!this._hasReadData){
            var queryRecord :M_TDHMJ_GameMessage.CMD_C_QueryGameRecord = new M_TDHMJ_GameMessage.CMD_C_QueryGameRecord();
            queryRecord.queryNum = 0;
            TDHMJ.ins.iclass.sendData(queryRecord);
            this.loadRecordData();
            // }else{
            //     this.loadRecordData();
            // }
        ...
        private loadRecordData():void{
            ...
            for(var i: number = 0;i < this._gameRecordAry.length;i++) {
                // var record: TDHMJ_SinglePlayerRecord = new TDHMJ_SinglePlayerRecord(this._gameRecordAry[i],i);
                // this._recordListContainer.addChild(record);
                // record.node.y = i * 61;
            //       let newnode = TDHMJ_JiFenBan._freeNode.get();
            // if (!cc.isValid(newnode)) {
            //     newnode = cc.instantiate(this.single);
            // }
            //   let sin=newnode.getComponent<TDHMJ_SinglePlayerRecord>(TDHMJ_SinglePlayerRecord);
            ...
            // this.group.addChild(newnode);
                ...
                for(var j:number=0; j<TDHMJMahjongDef.gPlayerNum; j++){
        ...
        private AddDataToContain(data:M_TDHMJ_GameMessage.GameRecordResult,i:number){
         //   var dt=cc.instantiate(this.pfb_SinglePlayerRecord);
          ...
          //  sin.init(this._gameRecordAry[i],i);
            ...
            newnode.y=i*61;
            var xx:TDHMJ_SinglePlayerRecord=newnode.getComponent<TDHMJ_SinglePlayerRecord>(TDHMJ_SinglePlayerRecord);
            xx.Init(data,i);
