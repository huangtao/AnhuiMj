...
@ccclass
export default class JZMJ_SelGang extends cc.Component {
     ...
     */
    private Gang_1(){
        // this.btn_gang_0.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_gangCard, this._gangCard[0]));
        M_JZMJClass.ins.OnGangSel(this._gangCard[0]);
        this.node.active = false;
    }
     ...
     */
    private Gang_2(){
        // this.btn_gang_1.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_gangCard, this._gangCard[1]));
        M_JZMJClass.ins.OnGangSel(this._gangCard[1]);
        this.node.active = false;
    }
     ...
     */
    private Gang_3(){
        // this.btn_gang_0.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_gangCard, this._gangCard[2]));
        M_JZMJClass.ins.OnGangSel(this._gangCard[2]);
        this.node.active = false;
    }
    ...
    public showGang(gangCard: Array<number>): void {
        ...
        for (var i: number = 0; i < gangCard.length; i++) {
            if (JZMJMahjongDef.gInvalidMahjongValue != gangCard[i]) {
