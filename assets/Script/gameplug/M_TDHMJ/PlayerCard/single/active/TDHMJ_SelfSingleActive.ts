...
@ccclass
export default class TDHMJ_SelfSingleActive extends TDHMJ_SingleActiveBase {
    ...
    private onTouchBegin(e:cc.Event.EventTouch):void{
        //cc.log("1111111");
        if(M_TDHMJClass.ins.ifCanOp && M_TDHMJClass.ins.IfCanOutACard(this._cardValue) && cc.isValid(e)){
    ...
    private onTouchEnd(e:cc.Event.EventTouch):void{
        ...
        if(this._isTouch){
            ...
            if(tempY >=30 ) {
                ...
                if(M_TDHMJClass.ins.ifCanOp)
                {   
                    //cc.log("33333");
                    this.node.opacity=0;
                    this.down();
                    this._isValid=false;
                    this.node.dispatchEvent(new TDHMJEvent(TDHMJEvent.msg_outACard,this._cardValue));
                    
                    //this.node.destroy();
                }
                else{
                    cc.log("44444");
                    this.node.dispatchEvent(new TDHMJEvent(TDHMJEvent.msg_reflashHandCard));
                }
            }
    ...
    public onClick():void{
        ...
        //如果已经胡牌
        if(TDHMJ.ins.iclass.isAlreadyHu()){
            return;
        }
        cc.log("点击了！"+this._cardValue);
        if(TDHMJMahjongDef.gInvalidMahjongValue != this._cardValue){
            
                if(M_TDHMJClass.ins.ifCanOp && M_TDHMJClass.ins.IfCanOutACard(this._cardValue)){
                    ...
                    if(this._isUp){//如果已经站立,打出去
                        ...
                        M_TDHMJClass.ins.showHideCard(this.cardValue);
                        if(this._ifCanTingAfterOutThisCard) {
                            //显示听牌
                            M_TDHMJClass.ins.showTingCard(this.cardValue,this.node.x,false);
                        }else{

                            //注释
                            // let index =0;
                            // if(!M_TDHMJClass.ins._tingPai[M_TDHMJClass.ins.SelfChair] && index == 0){
    ...
    public showBackCard():void{
        ...
        // this.bmp_cardback.spriteFrame=TDHMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        //this.bmp_cardback.spriteFrame=TDHMJ.ins.iclass.getMahjong3DPaiBeiRes("b_btm_self_mj_bg");
        if(TDHMJ.ins.iclass.is2D()){
            this.bmp_cardback.spriteFrame=TDHMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        }
        else{
            this.bmp_cardback.spriteFrame=TDHMJ.ins.iclass.getMahjong3DPaiBeiRes("chi_self_2_13");
        }
    ...
    public showCard(card: number,isLie: boolean,index:number,_hqmj=TDHMJ.ins.iclass): void {
        //cc.log("开始刷新牌元"+card);
