...
import HQMJ_SinglePlayerRecordX from "./HQMJ_SinglePlayerRecordX";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HQMJ_JiFenBanX extends cc.Component {
    ...
    public SetPlayerData():void{
        for(var i:number=0; i<HQMJMahjongDef.gPlayerNum; i++){
            LoadHeader(HQMJ.ins.iclass.getTablePlayerAry()[i].FaceID, this.img_accountAry[i]);
    ...
    private loadRecordData():void{
        ...
        for(let i: number = 0;i < this._gameRecordAry.length;i++) {
             let newnode = HQMJ_JiFenBanX._freeNode.get();
            if (!cc.isValid(newnode)) {
                newnode = cc.instantiate(this.single);}
           // let newnode=cc.instantiate(this.single);
            let sin=newnode.getComponent<HQMJ_SinglePlayerRecordX>(HQMJ_SinglePlayerRecordX);
