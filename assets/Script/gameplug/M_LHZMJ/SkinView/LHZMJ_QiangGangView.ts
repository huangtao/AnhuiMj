...
@ccclass
export default class LHZMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("LHZMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_LHZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new LHZMJEvent(LHZMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_LHZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new LHZMJEvent(LHZMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(LHZMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        this.img_card.spriteFrame=LHZMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
