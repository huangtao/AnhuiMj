...
@ccclass
export default class LQMJ_VideoActiveBase extends LQMJ_ActiveBase {
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        //     this._handCard.push(M_LQMJVideoClass.ins.getPlayerHandCardData(M_LQMJVideoClass.ins.logic2physicalChair(this._logicChair))[i]);//[holdIdx * 4 + i]);
        // }
        // this.handCardChange(M_LQMJVideoClass.ins);
    ...
    public playerHu(huCard: number): void {
        ...
        var chair : number = M_LQMJVideoClass.ins.logic2physicalChair(this._logicChair);
        
        var lastCard: Array<number> = new Array<number>();
        for(var i: number = 0;i < M_LQMJVideoClass.ins.getPlayerHandCardData(chair).length;i++) {
            lastCard.push(M_LQMJVideoClass.ins.getPlayerHandCardData(chair)[i]);
        }
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(let i: number = 0;i < newNum;i++) {
                let newnode = M_LQMJVideoClass.ins.getFreeActive(this._logicChair).get();
            ...
            for(let i: number = 0;i < delNum;i++) {
                ...
                M_LQMJVideoClass.ins.getFreeActive(this._logicChair).put(this._cardData[i].node);
            }
