...
@ccclass
export default class LQMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("LQMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_LQMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new LQMJEvent(LQMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_LQMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new LQMJEvent(LQMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(LQMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        //this.img_card.spriteFrame=LQMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
