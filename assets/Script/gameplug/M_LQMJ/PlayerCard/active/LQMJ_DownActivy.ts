import LQMJ_OtherActive from "./LQMJ_OtherActive";
...
@ccclass
export default class LQMJ_DownActive extends LQMJ_OtherActive {
    ...
    protected refreshHandCard(): void {
        ...
        if(LQMJ.ins.iclass.is2D()){
            ...
            if(this.isLie){
            //起始位置
            var startPos: number = LQMJ_DownActive.ArrangeStartPos_lie[this.fixedCardNum];
            ...
            //起始位置
            var startPos: number = LQMJ_DownActive.ArrangeStartPos_stand[this.fixedCardNum];
            for(var i: number = this._cardData.length;i > 0;i--) {
                this._cardData[i-1].node.setLocalZOrder(i);
            ...
            if(this.isLie){
                //起始位置
                let startPos: number = LQMJ_DownActive.ArrangeStartPos3D_lie[this.fixedCardNum];
                ...
                //起始位置
                let startPos: number = LQMJ_DownActive.ArrangeStartPos3D_stand[this.fixedCardNum];

                for(let i: number = this._cardData.length;i > 0;i--) {
                    this._cardData[i-1].node.setLocalZOrder(this._cardData.length-i+1);
                    // this._cardData[i - 1].node.x = 480;
                    // this._cardData[i - 1].node.y = startPos + idx * 32 -60;
                    // if(_lqmj == LQMJ.ins.iclass)
                        this._cardData[i - 1].showCard(this._handCard[i-1],this.isLie,++temp);
                    // else
