...
@ccclass
export default class LHZMJ_ReadyStatusUserInfo extends cc.Component {
    ...
    public init():void{
        cc.log("init LHZMJ_ReadyStatusUserInfo")
    ...
    public refreshInvite():void{
        ...
        for(var i:number=0; i<LHZMJMahjongDef.gPlayerNum-1; i++){
            ...
            var physicChair : number = LHZMJ.ins.iclass.logic2physicalChair(i + 1);
            
            //人人都有邀请功能
            if((LHZMJ.ins.iclass.isCreateRoom()) && (null == LHZMJ.ins.iclass.getTablePlayerAry()[LHZMJ.ins.iclass.logic2physicalChair(i + 1)])) {
    ...
    private onSelUserFace(logicChair: number): void {
        ...
        //M_LHZMJView.ins.UserData.showUserData(LHZMJ.ins.iclass.getTablePlayerAry()[chair],LHZMJ_ReadyStatusUserInfo.UserDataPos[logicChair].x,LHZMJ_ReadyStatusUserInfo.UserDataPos[logicChair].y);
       M_LHZMJView.ins.UserData.showUserData(LHZMJ.ins.iclass.getTableConfig().isValid,LHZMJ.ins.iclass.getTablePlayerAry()[chair],LHZMJ_ReadyStatusUserInfo.UserDataPos[logicChair].x,LHZMJ_ReadyStatusUserInfo.UserDataPos[logicChair].y);
    ...
    public set offlineChair(chair: number) {
        //this._img_offline[LHZMJ.ins.iclass.physical2logicChair(chair)].visible = true;
        // if(M_LHZMJClass.ins.isSelfCreateRoom && (M_LHZMJClass.ins.TableConfig.alreadyGameNum > 0)) {
        //     this._img_readyStatus[LHZMJ.ins.iclass.physical2logicChair(chair)].visible = true;
        // }
    ...
    public set reconnectChair(chair: number) {
        //this._img_offline[LHZMJ.ins.iclass.physical2logicChair(chair)].visible = false;
        this.userAry[LHZMJ.ins.iclass.physical2logicChair(chair)].Hideoffline();
    }                
                    

    private onShare():void{
        //var chair: number = LHZMJ.ins.iclass.logic2physicalChair(lgchair);
        //M_LHZMJView.ins.OnButtonShare();
        // M_LHZMJClass.ins.showShare("合肥麻将邀请",chair);
        //             console.log(`分享内容:${M_LHZMJClass.ins.ShareTitle}`);
    ...
    public OnPlayerStatusChange(chairID: number,newStatus: QL_Common.GState): void {
       ...
       // this.group_ready[logicChair].node.active = (QL_Common.GState.PlayerReady == newStatus);//|| Uyi.Common.GState.OfflineInGame == newStatus);
        ...
        if(QL_Common.GState.PlayerReady != newStatus)
        {
           ...
           // this.group_ready[logicChair].node.active = QL_Common.GState.OfflineInGame == newStatus;//LHZMJ.ins.iclass.getTablePlayerAry()[chairID].DiamondsNum>=M_LHZMJClass.ins.gameMoneyNum && Uyi.Common.GState.OfflineInGame == newStatus;
            if(!this.group_ready[logicChair].node.active)
            {
                this.group_ready[logicChair].node.active=LHZMJ.ins.iclass.getTableConfig().alreadyGameNum>0;
            }
        }
    ...
    public OnPlayerLeave(chairID: number): void {
        ...
        /*if(chairID != LHZMJ.ins.iclass.getSelfChair()){
            //显示邀请按钮
            this._btn_invite[logicChair - 1].visible = M_LHZMJClass.ins.SelfIsTableOwener;
        }*/
        if(logicChair > 0){
            this.group_invite[logicChair - 1].node.active = LHZMJ.ins.iclass.isCreateRoom();
        }
    }
    ...
    public set tableOwener(chair: number) {
        ...
        this.btn_kick.node.active = M_LHZMJClass.ins.SelfIsTableOwener;
        this.refreshInvite();
        /*
        if(M_LHZMJClass.ins.SelfIsTableOwener){//如果我是房主,刷新邀请按钮状态
            this.refreshInvite();
        }*/
    }
    ...
    public refreshSelfScore():void{
        this.userAry[0].SetMoney(this.getPlayerLeftMoneyNum(LHZMJ.ins.iclass.getTablePlayerAry()[LHZMJ.ins.iclass.getSelfChair()]));
        this.userAry[0].ShowMoney(!LHZMJ.ins.iclass.getTableConfig().needHideUserMoney);
    }
    ...
    private showChairPlayer(chairID: number,player: QL_Common.TablePlayer){
        ...
        //显示玩家账号
        let ss="";
        if(LHZMJMahjongAlgorithm1.strLen(player.NickName) > 16){
        ...
        //显示余额
        this.userAry[logicChair].SetMoney(this.getPlayerLeftMoneyNum(player));//showUserMoney(LHZMJ.ins.iclass.getRoomData().CheckMoneyType,this.getPlayerLeftMoneyNum(player));
        this.userAry[logicChair].ShowMoney(!LHZMJ.ins.iclass.getTableConfig().needHideUserMoney);
        

        //不是自己,隐藏邀请按钮
        /*if(player.PlayerID != M_LHZMJClass.ins.userData.UserID) {
            //邀请按钮隐藏
    ...
    private onReady():void{
        ...
        if(M_LHZMJClass.ins.checkMoneyCanGame){
            ...
            {
                M_LHZMJView.ins.MsgBox.showMsgBox('余额不足请先充值！', "确定", () => {
                    //打开充值
        ...
        // else{                 
        //     var tipMsg: string = `您的游戏币已经不足,无法继续游戏,本次共需要:${TranslateMoneyTypeName(LHZMJ.ins.iclass.getRoomData().TableCostMoneyType)}X${M_LHZMJClass.ins.gameMoneyNum},点击关闭将会返回大厅。您还可以选择：`;
        //     if(M_LHZMJClass.ins.isSelfCreateRoom){
        //         //发起求助
        //         M_LHZMJView.ins.MsgBox.showMsgBox(tipMsg,"去充值",() => {
        //             //充值 
    ...
    private onKickUser(chair:number):void{
        ...
        if(null == LHZMJ.ins.iclass.getTablePlayerAry()[phychair]){
            M_LHZMJView.ins.TipMsgView.showTip("此位置没有玩家");
            return;
        }
        ...
        if(LHZMJ.ins.iclass.getTablePlayerAry()[chair].VIPLV > LHZMJ.ins.iclass.getTablePlayerAry()[selfChair].VIPLV){
            M_LHZMJView.ins.MsgBox.showMsgBox("不可踢出VIP等级高于自己的玩家");
            return;
        }
        
        M_LHZMJView.ins.MsgBox.showMsgBox(`是否踢出玩家${LHZMJ.ins.iclass.getTablePlayerAry()[chair].NickName}?`,"确定",()=>{
            M_LHZMJClass.ins.PleaseLeavePlayer(LHZMJ.ins.iclass.getTablePlayerAry()[chair].PlayerID);
            this.group_kickuser.active=false;
        },this,"取消");
    }
