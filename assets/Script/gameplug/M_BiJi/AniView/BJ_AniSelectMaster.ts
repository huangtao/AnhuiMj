...
import { StageWidth, StageHeight, CommonTexturePath } from "../GameHelp/BJ_GameHelp";
import { BiJi } from "../GameHelp/BJ_IBiJiClass";
import SkinAniSelectMaster from "./BJ_SkinAniSelectMaster";
...
export default class AniSelectMaster extends cc.Component {
     ...
     * 下注开始消息
     */
    private msg_betStart: M_BiJi_GameMessage.CMD_S_BetStart;
    /**
     * 动画容器
     */
    ...
    public Destroy() {
        ...
        if (this.msg_betStart != null) {
            BiJi.ins.iview.Rec_BetStart(this.msg_betStart, true);
            this.msg_betStart = null;
        }
        this.Clear();
    }
     ...
     * 显示动画
     */
    public ShowAni(data: M_BiJi_GameMessage.CMD_S_BetStart) {
    ...
    private SelectMaster(aniCount: number) {
        ...
        var chair = BiJi.ins.iclass.GetClientChair(serverChair);
