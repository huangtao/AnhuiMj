...
import JZMJ_SinglePlayerRecordX from "./JZMJ_SinglePlayerRecordX";

const { ccclass, property } = cc._decorator;

@ccclass
export default class JZMJ_JiFenBanX extends cc.Component {
    ...
    public SetPlayerData():void{
        for(var i:number=0; i<JZMJMahjongDef.gPlayerNum; i++){
            LoadHeader(JZMJ.ins.iclass.getTablePlayerAry()[i].FaceID, this.img_accountAry[i]);
    ...
    private loadRecordData():void{
        ...
        for(let i: number = 0;i < this._gameRecordAry.length;i++) {
             let newnode = JZMJ_JiFenBanX._freeNode.get();
            if (!cc.isValid(newnode)) {
                newnode = cc.instantiate(this.single);}
           // let newnode=cc.instantiate(this.single);
            let sin=newnode.getComponent<JZMJ_SinglePlayerRecordX>(JZMJ_SinglePlayerRecordX);
