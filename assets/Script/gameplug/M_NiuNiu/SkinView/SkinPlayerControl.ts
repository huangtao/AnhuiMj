import SkinPlayer from "./SkinPlayer";
import { PlayerCount, UserState, LastCardIndex, GameStage } from "../GameHelp/GameHelp";
import { QL_Common } from "../../../CommonSrc/QL_Common";
import { M_NiuNiu_GameMessage } from "../../../CommonSrc/M_NiuNiu_GameMessage";
import AniDealControl from "../AniView/AniDealControl";
import AniPlayerDealLast from "../AniView/AniPlayerDealLast";
import AniFlip from "../AniView/AniFlip";
import AniSelectMaster from "../AniView/AniSelectMaster";
import SkinAni from "../AniView/SkinAni";
import AniResult from "../AniView/AniResult";
...
export default class SkinPlayerControl extends cc.Component {
    ...
    public SetUserMaster(chair: number) {
        this.skinPlayer[chair].SetUserFrame(0);
        for (var i = 0; i < PlayerCount; i++) {
            if (this.skinPlayer[i].node.active && i != chair) {
                this.HideUserRobRate(i);
            }
        }
    }
    /**
    ...
    public SetUserSelectCards(chair: number) {
        if (!this.IsChairSafe(chair)) return;
        this.skinPlayer[chair].SetUserSelectCards();
        if (chair == 0) {
     ...
     * 隐藏所有玩家的边框
     */
    public HideAllUserFrame() {
        for (var i = 0; i < this.skinPlayer.length; i++) {
            this.skinPlayer[i].HideUserFrame();
        }
    }
    /**
     * 设置房主
    ...
    //再讲CMD_S_GameStart消息传入M_NiuNiuView中，在根据游戏模式判断是显示抢庄按钮或下注按钮

    /**
     * 播放发牌动画
     */
    public AniDeal(data: M_NiuNiu_GameMessage.CMD_S_GameStart) {
     ...
     * 播放选庄动画
     */
    public AniMaster(data: M_NiuNiu_GameMessage.CMD_S_BetStart) {
        this.aniSelectMaster.ShowAni(data);
    }
    /**
     * 播放发最后一张牌动画
     */
    public AniDealLast(data: M_NiuNiu_GameMessage.CMD_S_SelectCardsStart) {
        this.aniPlayerDealLast.ShowAni(data);
    }
    public AniFlip(data: M_NiuNiu_GameMessage.CMD_S_SelectCardsStart) {
        this.aniFlip.ShowAni(data);
    }
    /**
     * 播放结果动画
     */
    public AniResult(data: M_NiuNiu_GameMessage.CMD_S_GameResult) {
        this.aniResult.ShowAni(data);
    }
    /**
     * 设置计分板信息，以在结果动画播放完毕后弹出计分板
     */
    public AniScoreView(data: M_NiuNiu_GameMessage.CMD_S_ScoreView) {
        this.aniResult.SetScoreView(data);
    }
    /**
     * 设置庄家变更信息，以在结果动画播放完毕后弹出庄家变更消息
     */
    public AniMasterChange(data: M_NiuNiu_GameMessage.CMD_S_MasterChange) {
        this.aniResult.SetMasterChange(data);
    }
    /**
     * 设置玩家余额
     */
    public AniPlayerScore(data: M_NiuNiu_GameMessage.CMD_S_PlayerScore) {
    ...
    public GetPlayerInfoPoint(chair: number): cc.Vec2 {
        ...
        public GetPlayerChatPoint(chair: number): cc.Vec2 {
        var point = new cc.Vec2(this.skinPlayer[chair].node.x, this.skinPlayer[chair].node.y);
        if(chair == 3){
            point.x -=this.skinPlayer[chair].node.width / 2+20;
        }
        // switch (chair) {
        //     case 0: {
        //         point.y += this.skinPlayer[chair].node.height / 2;
    ...
    public GetCardPos(chair: number, index: number) {
        return this.skinPlayer[chair].GetCardPos(index);
    }
    //==================================== 辅助 结束 =======================================
