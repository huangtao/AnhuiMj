import { TingCardTip } from "../ConstDef/BBMJMahjongDef";
import M_BBMJClass from "../M_BBMJClass";
import BBMJ_SingleTingCardTip from "./BBMJ_SingleTingCardTip";

const { ccclass, property } = cc._decorator;

@ccclass
export default class BBMJ_TingTip extends cc.Component {
    ...
    @property(cc.Prefab)
    BBMJ_SingleTingCard_Tip: cc.Prefab=null;
    ...
    //结算
    private _singleTingCard: BBMJ_SingleTingCardTip;
    /**
     * 结算视图
     * */
    public get SingleTingCard(): BBMJ_SingleTingCardTip {
    ...
    //听牌提示
    private _tingCardAry: Array<BBMJ_SingleTingCardTip>;
        ...
        if(this._tingCardAry!=null)
        {
            while (this._tingCardAry.length > 0) {
               // this._tingCardAry.pop().node.destroy();
                BBMJ_TingTip._freeNode.put(this._tingCardAry.pop().node);
            }
        }else{
            this._tingCardAry = new Array<BBMJ_SingleTingCardTip>();
        }
    ...
    public showTingTip(tingTip: Array<TingCardTip>, tips:boolean): void {
       ...
       // this.img_bg.node.width = 90 + tingTip.length * 120 - 40;
        ...
        for (var i: number = 0; i < tingTip.length; i++) {
            //var singleTip: BBMJ_SingleTingCardTip = new BBMJ_SingleTingCardTip();

          //  let singleTip=cc.instantiate(this.BBMJ_SingleTingCard_Tip);
          let singleTip = BBMJ_TingTip._freeNode.get();
            if (!cc.isValid(singleTip)) {
                singleTip = cc.instantiate(this.BBMJ_SingleTingCard_Tip);
            }
            this._singleTingCard=singleTip.getComponent<BBMJ_SingleTingCardTip>(BBMJ_SingleTingCardTip);
