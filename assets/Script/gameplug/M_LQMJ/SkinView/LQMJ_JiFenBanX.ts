...
import LQMJ_SinglePlayerRecordX from "./LQMJ_SinglePlayerRecordX";

const { ccclass, property } = cc._decorator;

@ccclass
export default class LQMJ_JiFenBanX extends cc.Component {
    ...
    public SetPlayerData():void{
        for(var i:number=0; i<LQMJMahjongDef.gPlayerNum; i++){
            LoadHeader(LQMJ.ins.iclass.getTablePlayerAry()[i].FaceID, this.img_accountAry[i]);
    ...
    private loadRecordData():void{
        ...
        for(let i: number = 0;i < this._gameRecordAry.length;i++) {
             let newnode = LQMJ_JiFenBanX._freeNode.get();
            if (!cc.isValid(newnode)) {
                newnode = cc.instantiate(this.single);}
           // let newnode=cc.instantiate(this.single);
            let sin=newnode.getComponent<LQMJ_SinglePlayerRecordX>(LQMJ_SinglePlayerRecordX);
