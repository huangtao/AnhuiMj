...
import MGMJ_SinglePoolBase from "../single/MGMJ_SinglePoolBase";
...
@ccclass
export default class MGMJ_PoolBase extends MGMJ_CardBase {
    ...
    //牌池数据
    protected _poolCard:Array<MGMJ_SinglePoolBase>;
    //牌阵数据
    ...
    public init(logicChair: number) {
        ...
        this._poolCard = new Array<MGMJ_SinglePoolBase>(0);
    ...
    public addPoolCard(card:number):{x:number,y:number}{
        let newnode = MGMJ.ins.iclass.getFreePool(this._logicChair).get();
        if (!cc.isValid(newnode)) {
            newnode = cc.instantiate(this.PoolType);
        }
        // var newnode=cc.instantiate(this.PoolType);
        var pool = newnode.getComponent<MGMJ_SinglePoolBase>(MGMJ_SinglePoolBase);
        pool.init();
    ...
    public delLastPoolCard(card:number,leftnum:number):{x:number,y:number}{
        ...
        MGMJ.ins.iclass.getFreePool(this._logicChair).put(this._poolCard.pop().node);
        return this.refreshPoolCard();
    }
    /**
     * 恢复牌池
    ...
    public recoveryPoolCard(cardAry:Array<number>):void{
        if((null != cardAry) && (cardAry.length > 0)){
            for(var i:number=0; i<cardAry.length; i++){
                ...
                M_MGMJClass.ins.RecordCard.outACard(cardAry[i]);
                let newnode = MGMJ.ins.iclass.getFreePool(this._logicChair).get();
                if (!cc.isValid(newnode)) {
                    newnode = cc.instantiate(this.PoolType);
                }
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // this._poolCard.pop().node.destroy();
            MGMJ.ins.iclass.getFreePool(this._logicChair).put(this.node.children[0]);
        }
    }
}
