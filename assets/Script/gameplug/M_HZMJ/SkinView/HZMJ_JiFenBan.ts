...
import HZMJ_SinglePlayerRecord from "./HZMJ_SinglePlayerRecord";
import M_HZMJClass from "../M_HZMJClass";
import HZMJ_HeadPortrait from "./HZMJ_HeadPortrait";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HZMJ_JiFenBan extends cc.Component {
    @property(cc.Button)
    ...
    private _touxiangAry:Array<HZMJ_HeadPortrait>;
    ...
    private InitTouXiang():void{
        this._touxiangAry=new Array<HZMJ_HeadPortrait>();
        for(var i=0;i<this.pfb_touXiang.length;i++){
            var toux=cc.instantiate(this.pfb_touXiang[i]);
            this._touxiangAry.push(toux.getComponent<HZMJ_HeadPortrait>(HZMJ_HeadPortrait));
    ...
    onLoad() {
        ...
        protected onShow(): void {
            ...
            for(var i:number=0; i<HZMJMahjongDef.gPlayerNum; i++){
                ...
                //this._img_accountAry[i].source=HZMJ.ins.iclass.getTablePlayerAry()[i].FaceID;

                this._touxiangAry[i].setHeadPortSpri(HZMJ.ins.iclass.getTablePlayerAry()[i].FaceID);
        ...
        private loadRecordData():void{
            ...
            for(var i: number = 0;i < this._gameRecordAry.length;i++) {
                ...
                // record.node.y = i * 61;
            //       let newnode = HZMJ_JiFenBan._freeNode.get();
            // if (!cc.isValid(newnode)) {
            //     newnode = cc.instantiate(this.single);
            // }
            //   let sin=newnode.getComponent<HZMJ_SinglePlayerRecord>(HZMJ_SinglePlayerRecord);
        ...
        private AddDataToContain(data:M_HZMJ_GameMessage.GameRecordResult,i:number){
         //   var dt=cc.instantiate(this.pfb_SinglePlayerRecord);
          ...
          //  sin.init(this._gameRecordAry[i],i);
            ...
            newnode.y=i*61;
            var xx:HZMJ_SinglePlayerRecord=newnode.getComponent<HZMJ_SinglePlayerRecord>(HZMJ_SinglePlayerRecord);
            xx.Init(data,i);
