import { SkinDissolveTableParam, StageWidth, StageHeight } from "../GameHelp/BJ_GameHelp";
    ...
    private ShowDissolveTable(msg: GameIF.CustomMessage,face:string[]) {
        ...
        if (data.result == 0) {
            ...
            if (data.chair >= 0 && data.chair < 6)
                chair = BiJi.ins.iclass.GetClientChair(data.chair);
            var sponsor = BiJi.ins.iclass.GetClientChair(data.sponsor);
            if (sponsor == chair) {
                this.node.active = true;
                M_BiJiView.Instance.TimePause();
                ...
                if (data.timer > 0) {
                    if (sponsor == 0) {
                        ...
                        let index = data.member.indexOf(M_BiJiClass.Instance.ChairID);                       
            ...
            M_BiJiView.Instance.OnDisTableSuccess();
            for (var i = 0; i < data.member.length; i++) {
                if (data.memberflag[i]) {
                   // this.label[i].string = data.memberName+"";
                    this.vote[i].string = "已同意";
            ...
            this.unscheduleAllCallbacks();
            this.node.active = false;
            M_BiJiView.Instance.TimeResume();
            var str = "玩家【" + data.chairName + "】拒绝解散房间";
            M_BiJiClass.Instance.UiManager.ShowMsgBox(str);
        }
    }
    private TimerHandle() {
        this._timer--;
