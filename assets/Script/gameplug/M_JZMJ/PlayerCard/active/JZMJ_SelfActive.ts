...
@ccclass
export default class JZMJ_SelfActive extends JZMJ_ActiveBase {

    onLoad() {
        // init logic
        super.onLoad();
        this.init(0);
        this.node.on(JZMJEvent.JZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        for(var i: number = 0;i < holdNum;i++) {
            this._handCard.push(JZMJ.ins.iclass.getSelfHandCardData()[i]);//[holdIdx*4 + i]);
        }
            ...
            for(let i: number = 0;i < upCard.length;i++) {
                for(let j: number = 0;j < this._cardData.length;j++) {
                    if(upCard[i] == this._cardData[j].cardValue) {
                        (<JZMJ_SelfSingleActive>this._cardData[j]).setUp();
                        break;
                    }                      
                }
            }
    ...
    public setUpCardDown():void{
        for(let j: number = 0;j < this._cardData.length;j++) {
            (<JZMJ_SelfSingleActive>this._cardData[j]).setDown();
        }
    }
    /**
     * 设置听牌标志牌
    ...
    public showTingCardToken(cardAry: Array<number>): void {
        if((null == cardAry) || (cardAry.length < 1)){
            for(var i: number = 0;i < this._cardData.length;i++) {
                (<JZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard = false;
            }
            return;
        }
        for(var i:number=0; i<this._cardData.length; i++){
            for(var j:number=0; j<cardAry.length; j++){
                if(this._cardData[i].cardValue == cardAry[j]){
                    (<JZMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard=true;
                    break;
                }
            }
        }
    ...
    public refreshCardStatus(): void {
            for(var j: number = 0;j < this._cardData.length;j++) {
                (<JZMJ_SelfSingleActive>this._cardData[j]).enable = true;
                (<JZMJ_SelfSingleActive>this._cardData[j]).showCoverBG = false;
            }
    }
    ...
    public arrangeHandCard(): void {
        ...
        // for(var i: number = 0;i < this._cardData.length;i++) {
        //     (<JZMJ_SelfSingleActive>this._cardData[i]).showBackCard();
        // }
    ...
    private arrangeCardComplete():void{
        ...
        this.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_arrangeHandCardComplete)); 
    }
    ...
    public activeEnable(enable: boolean): void {
        for(var i:number=0; i<this._cardData.length; i++){
            (<JZMJ_SelfSingleActive>this._cardData[i]).enable =  enable;
            if(!enable){
                (<JZMJ_SelfSingleActive>this._cardData[i]).showCoverBG=false;
            }
        }
    }
    ...
    protected refreshHandCard(): void {
        ...
        if(JZMJ.ins.iclass.is2D()){
            //起始位置
            let startPos: number = JZMJ_SelfActive.ArrangeStartPos[this.fixedCardNum];
            //开始排版
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                ...
                if(this.isHoldAfter && (i == (this._cardData.length - 1))) {
                    ...
                    if(!this.isLie && !JZMJ.ins.iclass.isVideo())
            ...
            //起始位置
            let startPos: number = JZMJ_SelfActive.ArrangeStartPos3D[this.fixedCardNum];
            //开始排版
    ...
    //             //newnode.on(JZMJEvent.JZMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    //             //this.createSingleActiveCard(newnode);
    //             var active = newnode.getComponent<JZMJ_SingleActiveBase>(JZMJ_SingleActiveBase);
    //             active.init();
     ...
     * 单个活动牌事件
     * */
    private onSingleActiveEvent(e:JZMJEvent):void{
        
        switch(e.msgCode){
            case JZMJEvent.msg_reflashHandCard:{
                e.stopPropagation();
                ////cc.log("自己刷新！");
                this.refreshHandCard();
                break;
    ...
    private OutCardAfter(outindex:number):void{
        //JZMJMahjongAlgorithm.delCard(this._handCard,[card]);
        // //cc.log("位序调整");
        // let str1="";
        ...
        // }
        // //cc.log(str1);
        let tempCard:JZMJ_SingleActiveBase;
