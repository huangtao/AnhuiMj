...
import { MGMJMahjongAlgorithm } from "./MGMJMahjongAlgorithm";

const { ccclass, property } = cc._decorator;


export  class MGMJMahjongAlogrithmHaveHun {
    ...
    public static GetTingCardArray(srcAry:Array<number>,HunAry:Array<number>,flowerAry:Array<number>=null):Array<number>{
            ...
            for (var i = 0; i < MGMJMahjongDef.gMahjongCard.length; i++)
            {
                if(flowerAry!=null){
                    if(MGMJMahjongAlgorithm.IsContainsNumber(flowerAry,MGMJMahjongDef.gMahjongCard[i])){//花牌不提示在胡牌列表里面
                        continue;
                    }
                }
                ...
                allCard.push(MGMJMahjongDef.gMahjongCard[i]);
        ...
        public static CheckIfCanHuCardArrayForRQMJ(srcAry:Array<number>,HunAry:Array<number>):boolean{
          ...
          //  if (this.getLaiZiNum(srcAry) == 4)
            ...
            //    return true;
            //}
            if (MGMJMahjongAlogrithmHaveHun.IsHuNormalStructHaveHun(vSrc,HunAry))
            {
                return true;
            }
            return false;
        ...
        private static IsHuNormalStructHaveHun(vSrc:Array<number>,HunAry:Array<number>):boolean{
            ...
            {
                if (MGMJMahjongAlgorithm.IsContainsNumber(HunAry,vSrc[i]))
                    laiziNum++;
                else
                {
                    srcAry.push(vSrc[i]);
            ...
            MGMJMahjongAlgorithm.spiltHandCard(srcAry, vectorWan, vectorTong, vectorTiao, vectorZi);
            //3、得到各自的成组需赖子数
            var wanneed = MGMJMahjongAlgorithm.ChangeArrayToTripleForHuapai(vectorWan);
            var tongneed = MGMJMahjongAlgorithm.ChangeArrayToTripleForHuapai(vectorTong);
            var tiaoneed = MGMJMahjongAlgorithm.ChangeArrayToTripleForHuapai(vectorTiao);
            var zineed = MGMJMahjongAlgorithm.ChangeArrayToTripleForZipai(vectorZi);//红中赖子麻将实际上也没有字牌
            ...
            if (needNum <= laiziNum)
            {
                if (MGMJMahjongAlgorithm.ChangeArrayToPairAndTripleForHuapai(vectorWan, laiziNum - needNum))
                    return true;
            }
            ...
            if (needNum <= laiziNum)
            {
                if (MGMJMahjongAlgorithm.ChangeArrayToPairAndTripleForHuapai(vectorTong, laiziNum - needNum))
                    return true;
            }
            ...
            if (needNum <= laiziNum)
            {
                if (MGMJMahjongAlgorithm.ChangeArrayToPairAndTripleForHuapai(vectorTiao, laiziNum - needNum))
                    return true;
            }
            needNum = tongneed + tiaoneed + wanneed;
            if(needNum<=laiziNum)
            {
                if(MGMJMahjongAlgorithm.ChangeArrayToPairAndTripleForZipai(vectorZi,laiziNum-needNum))
                return  true;
            }
        ...
        public static ChangeArrayTo7PairsNeedHun(vectorCard:Array<number>,hunNum:number):boolean{
            ...
            if(MGMJMahjongAlgorithm.GetCardTypeNumFromCardAry(vectorCard)>7)//牌类型数大于7不可能组成7对
            {
                return false;
            }
            if(vectorCard==null)
            ...
            needHun = MGMJMahjongAlgorithm.ChangeArrayToPairsNeedHun(vectorCard);
       ...
       /////////////////////////////////////////////////////////////////////上面是获取出去某张牌后的听牌列表，下面是获取出哪些牌可以听牌//////////////////////////////////////////////////////////
        ...
        public static GetLastCardToTing(srcAry:Array<number>,HunAry:Array<number>):Array<number>{       
            ...
            for(var i:number=0;i<srcAry.length;i++)
            {
                ...
                {
                    //if(!MGMJMahjongAlgorithm.IsContainsNumber(HunAry,srcAry[i]))//这句是红中麻将中赖子牌不加入提示，因为赖子牌不能出，而赖子牌能出的宿州麻将则需要把这个判断去掉
