...
export default class HBMJ_SinglePlayerFX extends cc.Component {
    ...
    public SetPlayer(playidx:number):void{
        ...
        this._idx = playidx;
        let str:string = "";
        if(LQMJ.ins.iclass.getTablePlayerAry()[this._idx].NickName.length > 4){
            str = LQMJ.ins.iclass.getTablePlayerAry()[this._idx].NickName.substr(0,4);
        }else{
            str = LQMJ.ins.iclass.getTablePlayerAry()[this._idx].NickName;
        }
        this.lbl_play.string = str;
        this._player="/ID:" + `${LQMJ.ins.iclass.getTablePlayerAry()[this._idx].PlayerID}`;
        LoadHeader(M_LQMJClass.ins.TablePlayer[this._idx].FaceID, this.img_play);
