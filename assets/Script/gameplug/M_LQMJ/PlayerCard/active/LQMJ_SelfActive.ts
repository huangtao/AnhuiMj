...
@ccclass
export default class LQMJ_SelfActive extends LQMJ_ActiveBase {

    onLoad() {
        // init logic
        super.onLoad();
        this.init(0);
        this.node.on(LQMJEvent.LQMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        for(var i: number = 0;i < holdNum;i++) {
            this._handCard.push(LQMJ.ins.iclass.getSelfHandCardData()[i]);//[holdIdx*4 + i]);
        }
            ...
            for(let i: number = 0;i < upCard.length;i++) {
                for(let j: number = 0;j < this._cardData.length;j++) {
                    if(upCard[i] == this._cardData[j].cardValue) {
                        (<LQMJ_SelfSingleActive>this._cardData[j]).setUp();
                        break;
                    }                      
                }
            }
    ...
    public setUpCardDown():void{
        for(let j: number = 0;j < this._cardData.length;j++) {
            (<LQMJ_SelfSingleActive>this._cardData[j]).setDown();
        }
    }
    /**
     * 设置听牌标志牌
    ...
    public showTingCardToken(cardAry: Array<number>): void {
        if((null == cardAry) || (cardAry.length < 1)){
            ...
            for(var i: number = 0;i < this._cardData.length;i++) {
                (<LQMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard = false;
            }
        ...
        for(var i:number=0; i<this._cardData.length; i++){
            for(var j:number=0; j<cardAry.length; j++){
                if(this._cardData[i].cardValue == cardAry[j]){
                    (<LQMJ_SelfSingleActive>this._cardData[i]).ifCanTingAfterOutThisCard=true;
                    break;
                }
            }
        }
    ...
    public refreshCardStatus(): void {
            for(var j: number = 0;j < this._cardData.length;j++) {
                (<LQMJ_SelfSingleActive>this._cardData[j]).enable = true;
                (<LQMJ_SelfSingleActive>this._cardData[j]).showCoverBG = false;
            }
    }
    ...
    public arrangeHandCard(): void {
        ...
        // for(var i: number = 0;i < this._cardData.length;i++) {
        //     (<LQMJ_SelfSingleActive>this._cardData[i]).showBackCard();
        // }
    ...
    private arrangeCardComplete():void{
        ...
        this.node.dispatchEvent(new LQMJEvent(LQMJEvent.msg_arrangeHandCardComplete)); 
    }
    ...
    public activeEnable(enable: boolean): void {
        for(var i:number=0; i<this._cardData.length; i++){
            (<LQMJ_SelfSingleActive>this._cardData[i]).enable =  enable;
            if(!enable){
                (<LQMJ_SelfSingleActive>this._cardData[i]).showCoverBG=false;
            }
        }
    }
    ...
    protected refreshHandCard(): void {
        ...
        if(LQMJ.ins.iclass.is2D()){
            //起始位置
            let startPos: number = LQMJ_SelfActive.ArrangeStartPos[this.fixedCardNum];
            //开始排版
            for(var i: number = 0;i < this._cardData.length;i++) {
                ...
                if(this.isHoldAfter && (i == (this._cardData.length - 1))) {
                    ...
                    if(!this.isLie && !LQMJ.ins.iclass.isVideo())
            ...
            //起始位置
            let startPos: number = LQMJ_SelfActive.ArrangeStartPos3D[this.fixedCardNum];
            //开始排版
    ...
    //             //newnode.on(LQMJEvent.LQMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    //             //this.createSingleActiveCard(newnode);
    //             var active = newnode.getComponent<LQMJ_SingleActiveBase>(LQMJ_SingleActiveBase);
    //             active.init();
     ...
     * 单个活动牌事件
     * */
    private onSingleActiveEvent(e:LQMJEvent):void{
        
        switch(e.msgCode){
            case LQMJEvent.msg_reflashHandCard:{
                e.stopPropagation();
                ////cc.log("自己刷新！");
                this.refreshHandCard();
                break;
    ...
    protected createSingleActiveCard(p:cc.Node): void {
        //p.on(LQMJEvent.LQMJ_EVENT_TYPE,this.onSingleActiveEvent,this);
    }
    ...
    private OutCardAfter(outindex:number):void{
        //LQMJMahjongAlgorithm.delCard(this._handCard,[card]);
        // //cc.log("位序调整");
        // let str1="";
        ...
        // }
        // //cc.log(str1);
        let tempCard:LQMJ_SingleActiveBase;
