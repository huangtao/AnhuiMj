...
import { StageWidth, StageHeight, CardsCount, GameModel, CardWidth, CardHeight } from "../GameHelp/BJ_GameHelp";
...
export default class AniPlayerDeal extends cc.Component {
    ...
    public ShowAni(value: number[], obj1: any = null, fun1: () => void = null) {
        ...
        this.schedule(this.AniChild, 0.02 * BiJi.ins.iclass.GetSpeed());
    }
    private AniChild() {
        this.index++;
    ...
    private CardFlay() {
        ...
        skinCard.Show(startPos, tarPos, 0.3 * BiJi.ins.iclass.GetSpeed(), this.playerChair, tarScale, this, this.AniCardFlyOver);
    }
    /**
     * 牌飞动画结束
     */
    ...
    private AniExtendCards() {
        ...
        this.schedule(this.AniExtendCardsChild, 0.02 * BiJi.ins.iclass.GetSpeed());
    }
    ...
    private ExtendCard() {
        ...
        var time = tarindex * 0.02 * BiJi.ins.iclass.GetSpeed();
    ...
    private AniFlip() {
        ...
        if (GameLogic.IsGameModelRobMaster(this.cardsValue.length) && this.playerChair == 0) {
            ...
            SkinPlayerControl.Instance.skinPlayer[this.playerChair].skinCardsView.AniCardsFlip(this.cardsValue,0);
            this.scheduleOnce(this.AniOver, 0.3 * BiJi.ins.iclass.GetSpeed());
