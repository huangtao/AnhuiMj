...
import HQMJ_SingleActive from "../single/banlanceShow/HQMJ_SingleActive";
import { ReportError } from "../../../../Tools/Function";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HQMJ_BanlanceActive extends HQMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<HQMJ_SingleActive>;
    /**
     * 手牌
     * */
    ...
    public init() {
        this._cardData = new Array<HQMJ_SingleActive>(0);
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(var i: number = 0;i < newNum;i++) {
              ...
              let newnode = HQMJ_BanlanceActive._freeNode.get();
                if (!cc.isValid(newnode)) {
                    newnode = cc.instantiate(this.CardType);
                }
                this.node.addChild(newnode);
                var active = newnode.getComponent<HQMJ_SingleActive>(HQMJ_SingleActive);
                active.init();
            ...
            for(var i: number = 0;i < delNum;i++) {
             //   this._cardData[i].node.destroy();
              HQMJ_BanlanceActive._freeNode.put(this._cardData[i].node);
            }
    ...
    public clear(): void {
        ...
        while (this.node.children.length > 0) {
            ...
            if (cc.isValid(active)) {
                ...
                //     active.destroy();
                // } else {
                    HQMJ_BanlanceActive._freeNode.put(active);
                // }
            } else {
                ReportError("手牌数据出错！！！");
            }
