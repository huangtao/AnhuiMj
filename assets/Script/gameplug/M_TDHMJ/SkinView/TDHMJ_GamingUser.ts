...
@ccclass
export default class TDHMJ_GamingUser extends cc.Component {
    ...
    public SetPlayer(player: QL_Common.TablePlayer):void{   
        ...
        let ss="";
        let le=TDHMJMahjongAlgorithm.strLen(player.NickName)
        if( le > 4){
    ...
    public SetScore(score:number):void{
        this.userScore.showUserScore(score,TDHMJ.ins.iclass.getTableConfig().isValid);
    }

    public AddScore(score:number):void{
        this.userScore.addUserScore(score,TDHMJ.ins.iclass.getTableConfig().isValid);
    }
    ...
    public SetLa(score:number):void{
        ...
        let url="";
        if(score>=0 && score<=2){
            url=`gameres/M_TDHMJ/Texture/LaPaoZuo/hong${score}`;
            console.log(url);
    ...
    public SetPao(score:number):void{
        let url="";
        if(score>=0 && score<=2){
            url=`gameres/M_TDHMJ/Texture/LaPaoZuo/huan${score}`;
