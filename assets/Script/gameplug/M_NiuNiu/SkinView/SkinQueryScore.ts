import SkinQueryScoreEle from "./SkinQueryScoreEle";
import SkinQueryScorePlayer from "./SkinQueryScorePlayer";
import { PlayerCount, SkinQueryScoreParam, ScoreView, TexturePath, StageWidth, StageHeight, CommonTexturePath } from "../GameHelp/GameHelp";
...
export default class SkinQueryScore extends cc.Component {
    ...
    // @property(SkinQueryScoreEle)
    // private eleTotal: SkinQueryScoreEle=null;
    @property(cc.Node)
    ...
    // @property(cc.Button)
    // private btn_screenshot: cc.Button=null;
    // @property(cc.Button)
    ...
    onLoad() {
       // this.eleTotal.Init("总计", 0, true);
        ...
        for (var i = 0; i < this.skinQueryScorePlayer.length; i++) {
            ...
            this.skinQueryScorePlayer[i].node.x = -320 + 150 * i;
        ...
        // this.btn_screenshot.node.on("click", this.OnButtonscreenShot, this);
        // this.btn_back.node.on("click", this.OnButtonClose, this);
    }
    onDestroy() {
        this.poolNode.clear();
    }
    ...
    public ShowQueryScore(scoreview: ScoreView, isExit: boolean, gameCount: number[]) {
        ...
        if (scoreview != null) {
            ...
            for (var i = scoreview.scorelist.length-1; i >=0; i--) {
                ...
                ele.Init((i + 1) + "", (i + 1));
                ele.SetData(scoreview.scorelist[i].data);
            ...
            if (scoreview.scorelist.length > 6) {
                this.group.height = scoreview.scorelist.length * 50;
            }
            else {
                this.group.height = 300;
            }
            var selfindex = scoreview.chairlist.indexOf(NiuNiu.ins.iclass.GetServerChair(0));
    ...
    private CreateMaxWin(index: number) {
        ...
        SetTextureRes(TexturePath + "logo_maxwin", img_maxwin);
    ...
    private OnButtonscreenShot() {
        NiuNiu.ins.iclass.ScreenShot(true,this.groupRoot);
    }
}
