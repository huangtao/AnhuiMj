...
@ccclass
export default class HZMJ_ActiveBase extends HZMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<HZMJ_SingleActiveBase>;
    /**
     * 手牌
     * */
    ...
    public init(logicChair: number):void{
        ...
        this._cardData = new Array<HZMJ_SingleActiveBase>(0);
    ...
    public get isHoldAfter():boolean{
        return this._holdCard != HZMJMahjongDef.gInvalidMahjongValue;
    }
    ...
    public refreshHandCardData(cardAry:Array<number>):void{
        ...
        if(!HZMJMahjongAlgorithm.IsSame(cardAry,this._handCard))
        {
    ...
    public outACard(card: number): void {
        ...
        if(card==this._holdCard){
            ...
            HZMJ.ins.iclass.getFreeActive(this._logicChair).put(this._cardData.pop().node);
    ...
    private delCard(cardAry:Array<number>):void{
        ...
        HZMJMahjongAlgorithm.delCard(this._handCard,cardAry);
    ...
    public showLieCard(cardAry:Array<number>,huCard:number):void{
        ...
        if(HZMJMahjongDef.gInvalidMahjongValue != huCard){
    ...
    public playerHu(huCard: number):void{
        ...
        //如果抓到的牌不为空,那么就是自摸
        if(HZMJMahjongDef.gInvalidMahjongValue != this._holdCard){
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(let i: number = 0;i < newNum;i++) {
                let newnode = HZMJ.ins.iclass.getFreeActive(this._logicChair).get();
                ...
                let active = newnode.getComponent<HZMJ_SingleActiveBase>(HZMJ_SingleActiveBase);
                active.init();
            ...
            for(let i: number = 0;i < delNum;i++) {
                ...
                HZMJ.ins.iclass.getFreeActive(this._logicChair).put(this._cardData[i].node);
            }
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // let active: HZMJ_SingleActiveBase = this._cardData.pop();
            // active.node.destroy();
            HZMJ.ins.iclass.getFreeActive(this._logicChair).put(this.node.children[0]);
        }
    ...
    public DelNullData():void{
        ////cc.log("开始清空");
        let temp:Array<HZMJ_SingleActiveBase>= new Array<HZMJ_SingleActiveBase>();
        while(this._cardData.length > 0) {
            //cc.log(this._cardData.length);
            let active: HZMJ_SingleActiveBase = this._cardData.shift();
            if(active.IsValidValue())
            {
                ...
                // active.node.destroy();
                HZMJ.ins.iclass.getFreeActive(this._logicChair).put(active.node);
                // active.destroy();
            }
        }
        while(temp.length > 0) {
