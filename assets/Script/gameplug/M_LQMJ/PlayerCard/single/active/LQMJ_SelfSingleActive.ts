...
@ccclass
export default class LQMJ_SelfSingleActive extends LQMJ_SingleActiveBase {
    ...
    private onTouchBegin(e:cc.Event.EventTouch):void{
        //cc.log("1111111");
        if(M_LQMJClass.ins.ifCanOp && M_LQMJClass.ins.IfCanOutACard(this._cardValue) && cc.isValid(e)){
    ...
    private onTouchEnd(e:cc.Event.EventTouch):void{
        ...
        if(this._isTouch){
            ...
            if(tempY >=30 ) {
                ...
                if(M_LQMJClass.ins.ifCanOp)
                {   
                    //cc.log("33333");
                    this.node.opacity=0;
                    this.down();
                    this._isValid=false;
                    this.node.dispatchEvent(new LQMJEvent(LQMJEvent.msg_outACard,this._cardValue));
                    
                    //this.node.destroy();
                }
                else{
                    cc.log("44444");
                    this.node.dispatchEvent(new LQMJEvent(LQMJEvent.msg_reflashHandCard));
                }
            }
    ...
    public onClick():void{
        ...
        //如果已经胡牌
        if(LQMJ.ins.iclass.isAlreadyHu()){
            return;
        }
        cc.log("点击了！"+this._cardValue);
        if(LQMJMahjongDef.gInvalidMahjongValue != this._cardValue){
            
                if(M_LQMJClass.ins.ifCanOp && M_LQMJClass.ins.IfCanOutACard(this._cardValue)){
                    ...
                    if(this._isUp){//如果已经站立,打出去
                        ...
                        M_LQMJClass.ins.showHideCard(this.cardValue);
                        if(this._ifCanTingAfterOutThisCard) {
                            //显示听牌
                            M_LQMJClass.ins.showTingCard(this.cardValue,this.node.x,false);
                        }else{

                            //注释
                            // let index =0;
                            // if(!M_LQMJClass.ins._tingPai[M_LQMJClass.ins.SelfChair] && index == 0){
    ...
    public showBackCard():void{
        ...
        // this.bmp_cardback.spriteFrame=LQMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        //this.bmp_cardback.spriteFrame=LQMJ.ins.iclass.getMahjong3DPaiBeiRes("b_btm_self_mj_bg");
        if(LQMJ.ins.iclass.is2D()){
            this.bmp_cardback.spriteFrame=LQMJ.ins.iclass.getMahjongPaiBeiRes("pb3_backcard_self_1280");
        }
        else{
            this.bmp_cardback.spriteFrame=LQMJ.ins.iclass.getMahjong3DPaiBeiRes("chi_self_2_13");
        }
    ...
    public showCard(card: number,isLie: boolean,index:number,_hqmj=LQMJ.ins.iclass): void {
        //cc.log("开始刷新牌元"+card);
