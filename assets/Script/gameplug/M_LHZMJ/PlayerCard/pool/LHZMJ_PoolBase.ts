...
import LHZMJ_SinglePoolBase from "../single/LHZMJ_SinglePoolBase";
...
@ccclass
export default class LHZMJ_PoolBase extends LHZMJ_CardBase {
    ...
    //牌池数据
    protected _poolCard:Array<LHZMJ_SinglePoolBase>;
    //牌阵数据
    ...
    public init(logicChair: number) {
        ...
        this._poolCard = new Array<LHZMJ_SinglePoolBase>(0);
    ...
    public addPoolCard(card:number):{x:number,y:number}{
        let newnode = LHZMJ.ins.iclass.getFreePool(this._logicChair).get();
        if (!cc.isValid(newnode)) {
            newnode = cc.instantiate(this.PoolType);
        }
        // var newnode=cc.instantiate(this.PoolType);
        var pool = newnode.getComponent<LHZMJ_SinglePoolBase>(LHZMJ_SinglePoolBase);
        pool.init();
    ...
    public delLastPoolCard(card:number,leftnum:number):{x:number,y:number}{
        ...
        LHZMJ.ins.iclass.getFreePool(this._logicChair).put(this._poolCard.pop().node);
        return this.refreshPoolCard();
    }
    /**
     * 恢复牌池
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // this._poolCard.pop().node.destroy();
            LHZMJ.ins.iclass.getFreePool(this._logicChair).put(this.node.children[0]);
        }
    }
}
