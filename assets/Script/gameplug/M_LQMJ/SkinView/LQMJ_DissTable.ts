...
@ccclass
export default class LQMJ_DissTable extends cc.Component {
    ...
    private onVoteDissTable(vote:number):void{
        ...
        var agree: M_LQMJ_GameMessage.CMD_C_VoteDissTable = new M_LQMJ_GameMessage.CMD_C_VoteDissTable();
        agree.voteResult = vote;
        M_LQMJClass.ins.SendGameData(agree);
    ...
    public playerDissTable(chair:number,playerVote:Array<number>,lefttime:number=60):void{
        ...
        this.btn_agree.node.active = chair != M_LQMJClass.ins.SelfChair;
        this.btn_reject.node.active = chair != M_LQMJClass.ins.SelfChair;
        ...
        if(this._timerNum<=0)
            this._timerNum=0;
        this._sponsor = chair;
        if(M_LQMJClass.ins.TablePlayer[chair].NickName.length > 4)
            M_LQMJClass.ins.TablePlayer[chair].NickName = M_LQMJClass.ins.TablePlayer[chair].NickName.substr(0,4);
        this.lbl_sponsor.string=`玩家【${M_LQMJClass.ins.TablePlayer[chair].NickName}】申请解散房间`;
        ...
        for(var i: number = 0;i < LQMJMahjongDef.gPlayerNum;i++) {
            if(i != this._sponsor) {
                if(M_LQMJClass.ins.TablePlayer[i].NickName.length > 4)
                    M_LQMJClass.ins.TablePlayer[i].NickName = M_LQMJClass.ins.TablePlayer[i].NickName.substr(0,4);
                idx++;
                this.lbl_playerStatusAry[idx-1].string = M_LQMJClass.ins.TablePlayer[i].NickName;
                this.lbl_playervote[idx-1].node.active =true;
                this.lbl_playervote[idx-1].string = "投票中";
                LoadHeader(M_LQMJClass.ins.TablePlayer[i].FaceID, this.img_headPhoto[idx-1]);
            }
        }
        ...
        if(null == playerVote){
          ...
          //  this.schedule(this.onTimer,1,this._timerNum,1);
            ...
            // if(0 == playerVote[M_LQMJClass.ins.SelfChair]) {
            //     //this._intervalIdx = egret.setInterval(this.onTimer,this,1000);
            //     this.schedule(this.onTimer,1,this._timerNum,1);
            // }
   ...
   //     this.lbl_playerStatusAry[idx].string = `玩家【${M_LQMJClass.ins.getTablePlayerAry()[chair].NickName}】${1 == vote ? "同意":"拒绝"}`;
        this.lbl_playervote[idx].string = `${1 == vote ? "同意":"拒绝"}`;
     if(chair==M_LQMJClass.ins.SelfChair && 1 == vote){
            this.btn_agree.node.active = false;
            this.btn_reject.node.active = false;
        }
        if(vote==2){
    ...
    private onTimer():void{
        ...
        --this._timerNum;
        this.lbl_timer.string= `${this._timerNum}`;
        if((0 == this._timerNum) && (this._sponsor != M_LQMJClass.ins.SelfChair)){
            this.onVoteDissTable(1);
        }
    }
