import { TingCardTip } from "../ConstDef/JZMJMahjongDef";
import M_JZMJClass from "../M_JZMJClass";
import JZMJ_SingleTingCardTip from "./JZMJ_SingleTingCardTip";

const { ccclass, property } = cc._decorator;

@ccclass
export default class JZMJ_TingTip extends cc.Component {
    ...
    @property(cc.Prefab)
    JZMJ_SingleTingCard_Tip: cc.Prefab=null;
    ...
    //结算
    private _singleTingCard: JZMJ_SingleTingCardTip;
    /**
     * 结算视图
     * */
    public get SingleTingCard(): JZMJ_SingleTingCardTip {
    ...
    //听牌提示
    private _tingCardAry: Array<JZMJ_SingleTingCardTip>;
        ...
        if(this._tingCardAry!=null)
        {
            while (this._tingCardAry.length > 0) {
               // this._tingCardAry.pop().node.destroy();
                JZMJ_TingTip._freeNode.put(this._tingCardAry.pop().node);
            }
        }else{
            this._tingCardAry = new Array<JZMJ_SingleTingCardTip>();
        }
    ...
    public showTingTip(tingTip: Array<TingCardTip>,tips:boolean): void {
       ...
       // M_JZMJView.ins.TingTip.node.x=-330;
        ...
        for (var i: number = 0; i < tingTip.length; i++) {
          let singleTip = JZMJ_TingTip._freeNode.get();
            if (!cc.isValid(singleTip)) {
                singleTip = cc.instantiate(this.JZMJ_SingleTingCard_Tip);
            }
            this._singleTingCard=singleTip.getComponent<JZMJ_SingleTingCardTip>(JZMJ_SingleTingCardTip);
