import { SetTextureRes, SetFontRes } from "../GameHelp/BJ_BiJiFunction";
import { TexturePath, CommonFontPath } from "../GameHelp/BJ_GameHelp";
...
export default class SkinAniWinGold extends cc.Component {
    ...
    public ShowResultScore(chair: number, score: number) {
       ...
       //     SetTextureRes(TexturePath + "niuniures/resultScoreBg_blue", this.img_score);
          ...
          var destPos = cc.p(this.label_score.node.x,this.label_score.node.y+ 50);
                    this.label_score.node.runAction(cc.sequence(cc.spawn(cc.moveTo(0.5,destPos),cc.scaleTo(1.4,0.8)),
                        cc.callFunc(function ()
                        {
                            cc.log("得分动画，回调开始执行。")
                            this.label_score.string = "";
                            this.label_score.node.y -=50;
                        }.bind(this),this)));
        var end = 0;
        if (chair == 0) {
            end = 25;
    ...
    private AniScore(chair: number) {
        ...
        var action = cc.sequence(cc.scaleTo(0.25 * BiJi.ins.iclass.GetSpeed(), 1, 1), cc.spawn(cc.moveTo(0.75 * BiJi.ins.iclass.GetSpeed(),
            tarPos), cc.fadeTo(0.75 * BiJi.ins.iclass.GetSpeed(), 255)), cc.callFunc(this.onFinished, this));
