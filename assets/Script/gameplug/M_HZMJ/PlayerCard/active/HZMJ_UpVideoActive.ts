...
@ccclass
export default class HZMJ_UpVideoActive extends HZMJ_VideoActiveBase {
    ...
    protected refreshHandCard(): void {
        //起始位置
        var startPos: number = HZMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];

        //开始排版
        
        if(HZMJ.ins.iclass.is2D()){
            //起始位置
            let startPos: number = HZMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];
