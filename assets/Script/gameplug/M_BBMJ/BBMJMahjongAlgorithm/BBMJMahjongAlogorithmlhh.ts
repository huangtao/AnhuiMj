...
import { BBMJMahjongDef, enHuCardType } from "../ConstDef/BBMJMahjongDef";
...
export class BBMJMahjongAlgorithm1Lhh {
        ...
        private static CardAryIfContain7Pairs(cardAry:Array<number>,have4Num:number):boolean
        {
            ...
            BBMJMahjongAlgorithm1.sortCardAry(cardAry);
            have4Num = 0;
            is7pairs = true;
        ...
        public static IsQingYiSe(Ary:Array<number>):boolean
        {
            ...
            var colorfirst:number = BBMJMahjongAlgorithm1.GetMahjongColor(Ary[0]);

            for(var i=0;i<Ary.length;i++)
            {
                if(colorfirst!= BBMJMahjongAlgorithm1.GetMahjongColor(Ary[i]))
                {
                    isqys = false;
                    break;
                }
        ...
        public static IsHunYiSe(Ary:Array<number>):boolean
        {
            ...
            var colorfirst:number = BBMJMahjongDef.gInvalidMahjongValue;

            if(!BBMJMahjongAlgorithm1.OneAryIsContainsOtherAryAtListOneMember(Ary,BBMJMahjongAlgorithm1.GetDNXB()))//保证牌值列表里至少有东南西北中的一个
            {
                return isqys;
            }

            for(var i=0;i<Ary.length;i++){
                if(BBMJMahjongAlgorithm1.GetMahjongLogicValue(Ary[i])>34)//出现中发白，春夏秋冬，梅兰竹菊，那么就不可能是混一色
                {
                    return isqys;
                }
                
                if(BBMJMahjongAlgorithm1.GetMahjongColor(Ary[i])<3)
                {
                    colorfirst = BBMJMahjongAlgorithm1.GetMahjongColor(Ary[i]);
                    break;
                }
            }
        ...
        public static IsDuiDuiHu(vSrc:Array<number>):boolean
        {
            ...
            BBMJMahjongAlgorithm1.sortCardAry(ary);

            var cardtype:Array<number> = BBMJMahjongAlgorithm1.GetCardTypeAryFromCardAry(ary);

            for(var i=0;i< cardtype.length;i++)
            {
                if(BBMJMahjongAlgorithm1.GetCardNumInCardAry(ary, cardtype[i])!=2)
                {
                    continue;
                }

                BBMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(ary, cardtype[i], 2);
                ...
                BBMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(cardtp,cardtype[i],1);

                var isduidh:boolean = true;

                for(var j=0;j< cardtp.length;j++)
                {
                    if(BBMJMahjongAlgorithm1.GetCardNumInCardAry(ary, cardtp[j])>=3)
                    {
                        continue;
                    }
                    isduidh = false;
                ...
                BBMJMahjongAlgorithm1.AddZhiDingNumMemberInAry(ary, cardtype[i], 2);
        ...
        public static IsHuKaZhang(Ary:Array<number>, huCard:number):boolean
        {
            ...
            if (Ary == null || Ary.length < 5 || huCard == BBMJMahjongDef.gInvalidMahjongValue|| BBMJMahjongAlgorithm1.GetMahjongColor(huCard)>=3
                ||BBMJMahjongAlgorithm1.GetMahjongValue(huCard)==1||BBMJMahjongAlgorithm1.GetMahjongValue(huCard)==9)
            {
                return ishudandiao;
            }
            ...
            BBMJMahjongAlgorithm1.sortCardAry(byary);


            if(!BBMJMahjongAlgorithm1.IsContainsNumber(byary,huCard-1)||!BBMJMahjongAlgorithm1.IsContainsNumber(byary,huCard+1)){
                return ishudandiao;
            }

            //下面的逻辑很仓促写出来的，严密性需要检验

            BBMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard - 1,1);
            BBMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard,1);
            BBMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard + 1,1);

            if(BBMJMahjongAlgorithm1.CheckIfCanHuCardArray(byary))
            {
                ishudandiao = true;
            }
        ...
        {
            if (chair == BBMJMahjongDef.gInvalidChar || chair > 3)
            {
                return BBMJMahjongDef.gInvalidChar;
            }
            if (chair == 0)
            {
                return 3;
        ...
        public static RemoveOneMember(Ary:Array<number>,card:number):void{
            ...
            if(!BBMJMahjongAlgorithm1.AryContainsOneOrNot(Ary,card)){
                return;
            }
            for(var i=0;i<Ary.length;i++){
                if(Ary[i]!=card){
        ...
        public static RemoveMember(Ary:Array<number>,card:number,length:number=3):void
        {
            ...
            for (var i=0;i<2*length-1;i++)
            {
                ...
                if (BBMJMahjongAlgorithm1.IsContainsNumber(Ary,cd))
                {
        ...
        public static IsContansFlowrCard(CardAry:Array<number>):boolean
        {
            ...
            for(var i=0;i<M_BBMJClass.ins.FlowerAry.length;i++){
                FlowerAry.push(M_BBMJClass.ins.FlowerAry[i]);
            }
            for (var i = 0; i < CardAry.length; i++)
            {
                if (BBMJMahjongAlgorithm1.IsContainsNumber(FlowerAry,CardAry[i]))
                {
                    iscf = true;
                }
            }
         ...
         * @param card 
         */
        public static IsFlowerCardBBMJ(card:number):boolean{
            var FlowerAry:Array<number> = new Array<number>();
            for(var i=0;i<M_BBMJClass.ins.FlowerAry.length;i++){
                FlowerAry.push(M_BBMJClass.ins.FlowerAry[i]);
            }
