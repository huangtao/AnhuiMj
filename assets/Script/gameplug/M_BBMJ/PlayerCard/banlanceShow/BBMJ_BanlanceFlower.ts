...
import BBMJ_SingleFlower from "../single/banlanceShow/BBMJ_SingleFlower";

const { ccclass, property } = cc._decorator;

@ccclass
export default class BBMJ_BanlanceFlower extends BBMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<BBMJ_SingleFlower>;
    /**
     * 手牌
     * */
    ...
    public init() {
        this._cardData = new Array<BBMJ_SingleFlower>(0);
    ...
    protected refreshHandCard(): void {
        ...
        for(var i: number = 0;i < (this._cardData.length-1)/10+1;i++) {
            ...
            // if(this._holdCard != BBMJMahjongDef.gInvalidMahjongValue && (i == (this._cardData.length - 1))) {
            //     this._cardData[i].node.x += 15;
            // }
        }}else{
            for(var i: number = 0;i < this._cardData.length;i++) {
            this._cardData[i].node.x = i * 27+350;
    ...
    protected handCardChange(): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(var i: number = 0;i < newNum;i++) {
                let newnode = BBMJ_BanlanceFlower._freeNode.get();
                if (!cc.isValid(newnode)) {
                    newnode = cc.instantiate(this.CardType);
                }
                //var newnode=cc.instantiate(this.CardType);
                this.node.addChild(newnode);
                var active = newnode.getComponent<BBMJ_SingleFlower>(BBMJ_SingleFlower);
                active.init();
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            ...
            BBMJ_BanlanceFlower._freeNode.put(this.node.children[0]);
        }
        this._handCard.splice(0,this._handCard.length);
        this._holdCard = BBMJMahjongDef.gInvalidMahjongValue;
        this._fixedCardNum = 0;
