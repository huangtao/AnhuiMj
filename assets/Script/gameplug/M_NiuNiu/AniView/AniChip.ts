...
import SkinChip from "../SkinView/SkinChip";
import SkinPlayerControl from "../SkinView/SkinPlayerControl";
import { StageWidth, StageHeight, PlayerCount } from "../GameHelp/GameHelp";
...
export default class AniChip extends cc.Component {
    ...
    private AniChipSingle(createCount: number, oriChair: number, tarChair: number) {
        ...
        if (this.aniIndex <= createCount) {
            ...
            var waittime = GameLogic.Random(10, 30) / 1000 * NiuNiu.ins.iclass.GetSpeed();
    ...
    private AniCreateChipAndFly(oriChair: number, tarChair: number) {
        ...
        var action = cc.sequence(cc.moveTo(0.4 * NiuNiu.ins.iclass.GetSpeed(), tarPoint.x, tarPoint.y), callAction);
