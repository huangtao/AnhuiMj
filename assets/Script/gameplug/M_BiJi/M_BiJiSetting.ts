...
@ccclass
export default class M_BiJiSetting extends CreateRoomSettingBase {
    public GetSetting() {
    ...
    private onCreateTable(): any {
        ...
        const createTable = new M_BiJi_GameData();
        if (this._isGroupGame)
        ...
        createTable.checkIP = false;
        createTable.extendBet = this.GetExtendBetType();
        createTable.rubCard = this.GetRubCardType();
        createTable.havedropCard = true;
        createTable.havexiScore = this.toggle_xiscore.isChecked;
