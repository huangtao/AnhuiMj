...
import BBMJ_SinglePoolBase from "../single/BBMJ_SinglePoolBase";
...
@ccclass
export default class BBMJ_PoolBase extends BBMJ_CardBase {
    ...
    //牌池数据
    protected _poolCard:Array<BBMJ_SinglePoolBase>;
    //牌阵数据
    ...
    public init(logicChair: number) {
        ...
        this._poolCard = new Array<BBMJ_SinglePoolBase>(0);
    ...
    public addPoolCard(card:number):{x:number,y:number}{
        let newnode = BBMJ.ins.iclass.getFreePool(this._logicChair).get();
        if (!cc.isValid(newnode)) {
            newnode = cc.instantiate(this.PoolType);
        }
        // var newnode=cc.instantiate(this.PoolType);
        var pool = newnode.getComponent<BBMJ_SinglePoolBase>(BBMJ_SinglePoolBase);
        pool.init();
    ...
    public delLastPoolCard(card:number,leftnum:number):{x:number,y:number}{
        ...
        BBMJ.ins.iclass.getFreePool(this._logicChair).put(this._poolCard.pop().node);
        return this.refreshPoolCard();
    }
    /**
     * 恢复牌池
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // this._poolCard.pop().node.destroy();
            BBMJ.ins.iclass.getFreePool(this._logicChair).put(this.node.children[0]);
        }
    }
}
