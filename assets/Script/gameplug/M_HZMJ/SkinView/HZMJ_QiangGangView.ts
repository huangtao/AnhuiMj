...
@ccclass
export default class HZMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("HZMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_HZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new HZMJEvent(HZMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_HZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new HZMJEvent(HZMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(HZMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        this.img_card.spriteFrame=HZMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
