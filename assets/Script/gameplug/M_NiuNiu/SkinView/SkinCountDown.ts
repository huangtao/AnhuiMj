...
import { TimeFlag } from "../GameHelp/GameHelp";
...
export default class SkinCountDown extends cc.Component {
    ...
    public Show(timer: number, start: number = 0) {
        ...
        this.schedule(this.ChangeFillRange, timer / this.count * NiuNiu.ins.iclass.GetSpeed());
    }
    public Close() {
        this.Destroy();
        this.node.active = false;
    ...
    private AniOver() {
        ...
        this.Destroy();
        this.node.active = false;
        NiuNiu.ins.iview.TimerOver(TimeFlag.SelectCards);
    }
    /**
     * 分为100次
     */
