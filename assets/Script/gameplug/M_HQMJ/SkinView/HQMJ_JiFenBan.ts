...
import HQMJ_SinglePlayerRecord from "./HQMJ_SinglePlayerRecord";
import M_HQMJClass from "../M_HQMJClass";
import HQMJ_HeadPortrait from "./HQMJ_HeadPortrait";

const { ccclass, property } = cc._decorator;

@ccclass
export default class HQMJ_JiFenBan extends cc.Component {
    @property(cc.Button)
    ...
    private _touxiangAry:Array<HQMJ_HeadPortrait>;
    ...
    //记录数据
    private _gameRecordAry:Array<M_HQMJ_GameMessage.GameRecordResult>;
    //是否取过数据
    ...
    public constructor() {
        ...
        this._gameRecordAry = new Array<M_HQMJ_GameMessage.GameRecordResult>();
    ...
    private InitTouXiang():void{
        this._touxiangAry=new Array<HQMJ_HeadPortrait>();
        for(var i=0;i<this.pfb_touXiang.length;i++){
            var toux=cc.instantiate(this.pfb_touXiang[i]);
            this._touxiangAry.push(toux.getComponent<HQMJ_HeadPortrait>(HQMJ_HeadPortrait));
    ...
    onLoad() {
        ...
        public gameRecord(recordData:Array<number>):void{
            ...
            var gameRecord: M_HQMJ_GameMessage.GameRecordResult = new M_HQMJ_GameMessage.GameRecordResult();
         ...
         * 游戏记录数据到来
         * */
        public gameRecordDataCome(recordData: Array<M_HQMJ_GameMessage.GameRecordResult>):void{
        ...
        protected onShow(): void {
            ...
            this.lbl_gamenum.string="局号："+HQMJ.ins.iclass.getTableConfig().TableCode;
            for(var i:number=0; i<HQMJMahjongDef.gPlayerNum; i++){


                //this._img_accountAry[i].source=HQMJ.ins.iclass.getTablePlayerAry()[i].FaceID;

                this._touxiangAry[i].setHeadPortSpri(HQMJ.ins.iclass.getTablePlayerAry()[i].FaceID);

                let name:string=HQMJ.ins.iclass.getTablePlayerAry()[i].NickName;
                if (name.length > 4){
                ...
                if (i==HQMJ.ins.iclass.getSelfChair()) {
            ...
            //if(!this._hasReadData){
            var queryRecord :M_HQMJ_GameMessage.CMD_C_QueryGameRecord = new M_HQMJ_GameMessage.CMD_C_QueryGameRecord();
            queryRecord.queryNum = 0;
            HQMJ.ins.iclass.sendData(queryRecord);
            this.loadRecordData();
            // }else{
            //     this.loadRecordData();
            // }
        ...
        private loadRecordData():void{
            ...
            for(var i: number = 0;i < this._gameRecordAry.length;i++) {
                // var record: HQMJ_SinglePlayerRecord = new HQMJ_SinglePlayerRecord(this._gameRecordAry[i],i);
                // this._recordListContainer.addChild(record);
                // record.node.y = i * 61;
            //       let newnode = HQMJ_JiFenBan._freeNode.get();
            // if (!cc.isValid(newnode)) {
            //     newnode = cc.instantiate(this.single);
            // }
            //   let sin=newnode.getComponent<HQMJ_SinglePlayerRecord>(HQMJ_SinglePlayerRecord);
        ...
        private AddDataToContain(data:M_HQMJ_GameMessage.GameRecordResult,i:number){
         //   var dt=cc.instantiate(this.pfb_SinglePlayerRecord);
          ...
          //  sin.init(this._gameRecordAry[i],i);
            ...
            newnode.y=i*61;
            var xx:HQMJ_SinglePlayerRecord=newnode.getComponent<HQMJ_SinglePlayerRecord>(HQMJ_SinglePlayerRecord);
            xx.Init(data,i);
