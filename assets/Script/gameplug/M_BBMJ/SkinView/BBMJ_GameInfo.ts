...
@ccclass
export default class BBMJ_GameInfo extends cc.Component {
    ...
    private changeSence(e,any:string){
        BBMJ.ins.iclass.canvaSwitchClickEvent(any);
    }
    ...
    public set tableCode(value:string){
        if(value.length > 0){
            ...
            if(BBMJ.ins.iclass.isVideo()){
                if(M_BBMJVideoClass.ins.TableConfig._groupid>0){
                    this.lbl_RoomInfo.string = "亲友圈房号:"+value;
                }else{
                     this.lbl_RoomInfo.string = "房号:"+value;
                }
            }else{
                if(M_BBMJClass.ins.TableConfig._groupid>0){
                    this.lbl_RoomInfo.string = "亲友圈房号:"+value;
                }else{
                     this.lbl_RoomInfo.string = "房号:"+value;
                }
    ...
    public holdCardOver():void{
        ...
        this.leftCardNum = BBMJMahjongDef.gCardCount_Package - (BBMJMahjongDef.gCardCount_Active - 1) * BBMJMahjongDef.gPlayerNum;
    }
