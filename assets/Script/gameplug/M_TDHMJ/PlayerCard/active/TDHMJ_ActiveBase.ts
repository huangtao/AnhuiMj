...
import TDHMJ_SingleActiveBase from "../single/TDHMJ_SingleActiveBase";
import M_TDHMJVideoClass from "../../M_TDHMJVideoClass";

const { ccclass, property } = cc._decorator;

@ccclass
export default class TDHMJ_ActiveBase extends TDHMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<TDHMJ_SingleActiveBase>;
    /**
     * 手牌
     * */
    ...
    public init(logicChair: number):void{
        ...
        this._cardData = new Array<TDHMJ_SingleActiveBase>(0);
        this._handCard = new Array<number>(0);
        this._holdCard=TDHMJMahjongDef.gInvalidMahjongValue;
    ...
    public get isHoldAfter():boolean{
        return this._holdCard != TDHMJMahjongDef.gInvalidMahjongValue;
    }
    ...
    public refreshHandCardData(cardAry:Array<number>):void{
        ...
        if(!TDHMJMahjongAlgorithm.IsSame(cardAry,this._handCard))
        {
     ...
     * 抓到一张牌
     * */
    public holdACard(card: number,_lqmj = TDHMJ.ins.iclass): void {
        this._holdCard = card;
     ...
     * 打出一张牌
     * */
    public outACard(card: number,_lqmj = TDHMJ.ins.iclass): void {
        //自动出牌优化
     ...
     * 吃了一张牌
     * */
    public chiACard(card:number,type:number,_lqmj = TDHMJ.ins.iclass):void{
        if(type == 0){
     ...
     * 吃了一张牌
     * */
    public chiACardOther(card:number,_lqmj = TDHMJ.ins.iclass):void{
     ...
     * 碰了一张牌
     * */
    public pengACard(card:number,_lqmj = TDHMJ.ins.iclass):void{
        this.delCard([card,card]);
        this._holdCard = TDHMJMahjongDef.gInvalidMahjongValue;
     ...
     * 明杠了一张牌
     * */
    public MGangACard(card: number,_lqmj = TDHMJ.ins.iclass): void {
     ...
     * 暗杠了一张牌
     * */
    public AGangACard(card: number,_lqmj = TDHMJ.ins.iclass): void {
     ...
     * 补杠了一张牌
     * */
    public BGangACard(card: number,_lqmj = TDHMJ.ins.iclass): void {
        this.delCard([card]);
        this._holdCard = TDHMJMahjongDef.gInvalidMahjongValue;
    ...
    private delCard(cardAry:Array<number>):void{
        ...
        TDHMJMahjongAlgorithm.delCard(this._handCard,cardAry);
    ...
    public showLieCard(cardAry:Array<number>,huCard:number):void{
        ...
        if(TDHMJMahjongDef.gInvalidMahjongValue != huCard){
    ...
    public playerHu(huCard: number):void{
        ...
        //如果抓到的牌不为空,那么就是自摸
        if(TDHMJMahjongDef.gInvalidMahjongValue != this._holdCard){
     ...
     * 牌阵变化
     * */
    protected handCardChange(_lqmj = TDHMJ.ins.iclass): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(let i: number = 0;i < newNum;i++) {
                ...
                let active = newnode.getComponent<TDHMJ_SingleActiveBase>(TDHMJ_SingleActiveBase);
                active.init();
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // let active: TDHMJ_SingleActiveBase = this._cardData.pop();
            // active.node.destroy();
            TDHMJ.ins.iclass.getFreeActive(this._logicChair).put(this.node.children[0]);
        }
        if(this._handCard)
            this._handCard.splice(0,this._handCard.length);
        this._holdCard = TDHMJMahjongDef.gInvalidMahjongValue;
    ...
    public DelNullData():void{
        ////cc.log("开始清空");
        let temp:Array<TDHMJ_SingleActiveBase>= new Array<TDHMJ_SingleActiveBase>();
        while(this._cardData.length > 0) {
            //cc.log(this._cardData.length);
            let active: TDHMJ_SingleActiveBase = this._cardData.shift();
            if(active.IsValidValue())
            {
                ...
                // active.node.destroy();
                TDHMJ.ins.iclass.getFreeActive(this._logicChair).put(active.node);
                // active.destroy();
            }
        }
        while(temp.length > 0) {
