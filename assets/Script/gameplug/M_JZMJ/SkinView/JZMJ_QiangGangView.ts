...
@ccclass
export default class JZMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("JZMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_JZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_JZMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new JZMJEvent(JZMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(JZMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        //this.img_card.spriteFrame=JZMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
