...
@ccclass
export default class MGMJ_TingTip extends cc.Component {
    ...
    @property(cc.Prefab)
    MGMJ_SingleTingCard_Tip: cc.Prefab=null;
    ...
    //结算
    private _singleTingCard: MGMJ_SingleTingCardTip;
    /**
     * 结算视图
     * */
    public get SingleTingCard(): MGMJ_SingleTingCardTip {
    ...
    //听牌提示
    private _tingCardAry: Array<MGMJ_SingleTingCardTip>;
    ...
    public init() {
        ...
        if(this._tingCardAry!=null)
        {
            ...
            this._tingCardAry = new Array<MGMJ_SingleTingCardTip>();
        }
    ...
    public showTingTip(tingTip: Array<TingCardTip>,tips:boolean): void {
       ...
       // M_MGMJView.ins.TingTip.node.x=-330;
        ...
        for (var i: number = 0; i < tingTip.length; i++) {
          let singleTip = MGMJ_TingTip._freeNode.get();
            if (!cc.isValid(singleTip)) {
                singleTip = cc.instantiate(this.MGMJ_SingleTingCard_Tip);
            }
            this._singleTingCard=singleTip.getComponent<MGMJ_SingleTingCardTip>(MGMJ_SingleTingCardTip);
