...
@ccclass
export default class JZMJ_ActiveBase extends JZMJ_CardBase {
     ...
     * 显示牌
     * */
    protected _cardData: Array<JZMJ_SingleActiveBase>;
    /**
     * 手牌
     * */
    ...
    public init(logicChair: number):void{
        ...
        this._cardData = new Array<JZMJ_SingleActiveBase>(0);
        this._handCard = new Array<number>(0);
        this._holdCard=JZMJMahjongDef.gInvalidMahjongValue;
    ...
    public get isHoldAfter():boolean{
        return this._holdCard != JZMJMahjongDef.gInvalidMahjongValue;
    }
     ...
     * 抓到一张牌
     * */
    public holdACard(card: number,_JZMJ = JZMJ.ins.iclass): void {
        this._holdCard = card;
        this._handCard.push(card);
        this.handCardChange(_JZMJ);
    }

    /**
     * 打出一张牌
     * */
    public outACard(card: number,_JZMJ = JZMJ.ins.iclass): void {
        //自动出牌优化
        if(card==this._holdCard){
            this._handCard.pop();
            this._holdCard =JZMJMahjongDef.gInvalidMahjongValue;
            // this._cardData.pop().node.destroy();
            _JZMJ.getFreeActive(this._logicChair).put(this._cardData.pop().node);
     ...
     * 吃了一张牌
     * */
    public chiACard(card:number,type:number,_JZMJ = JZMJ.ins.iclass):void{
        if(type == 0){
     ...
     * 吃了一张牌
     * */
    public chiACardOther(card:number,_JZMJ = JZMJ.ins.iclass):void{
     ...
     * 碰了一张牌
     * */
    public pengACard(card:number,_JZMJ = JZMJ.ins.iclass):void{
        this.delCard([card,card]);
        this._holdCard = JZMJMahjongDef.gInvalidMahjongValue;
     ...
     * 明杠了一张牌
     * */
    public MGangACard(card: number,_JZMJ = JZMJ.ins.iclass): void {
     ...
     * 暗杠了一张牌
     * */
    public AGangACard(card: number,_JZMJ = JZMJ.ins.iclass): void {
     ...
     * 补杠了一张牌
     * */
    public BGangACard(card: number,_JZMJ = JZMJ.ins.iclass): void {
        this.delCard([card]);
        this._holdCard = JZMJMahjongDef.gInvalidMahjongValue;
    ...
    private delCard(cardAry:Array<number>):void{
        ...
        JZMJMahjongAlgorithm.delCard(this._handCard,cardAry);
    ...
    public showLieCard(cardAry:Array<number>,huCard:number):void{
        ...
        if(JZMJMahjongDef.gInvalidMahjongValue != huCard){
    ...
    public playerHu(huCard: number):void{
        ...
        //如果抓到的牌不为空,那么就是自摸
        if(JZMJMahjongDef.gInvalidMahjongValue != this._holdCard){
     ...
     * 牌阵变化
     * */
    protected handCardChange(_JZMJ = JZMJ.ins.iclass): void {
        ...
        if(this._handCard.length > this._cardData.length) {//需要增加活动牌
            ...
            for(let i: number = 0;i < newNum;i++) {
                let newnode = _JZMJ.getFreeActive(this._logicChair).get();
            ...
            for(let i: number = 0;i < delNum;i++) {
                ...
                _JZMJ.getFreeActive(this._logicChair).put(this._cardData[i].node);
            }
    ...
    public clear(): void {
        ...
        while(this.node.children.length > 0) {
            // let active: JZMJ_SingleActiveBase = this._cardData.pop();
            // active.node.destroy();
            JZMJ.ins.iclass.getFreeActive(this._logicChair).put(this.node.children[0]);
        }
        if(this._handCard)
            this._handCard.splice(0,this._handCard.length);
        this._holdCard = JZMJMahjongDef.gInvalidMahjongValue;
    ...
    public DelNullData():void{
        ////cc.log("开始清空");
        let temp:Array<JZMJ_SingleActiveBase>= new Array<JZMJ_SingleActiveBase>();
        while(this._cardData.length > 0) {
            //cc.log(this._cardData.length);
            let active: JZMJ_SingleActiveBase = this._cardData.shift();
            if(active.IsValidValue())
            {
                ...
                // active.node.destroy();
                JZMJ.ins.iclass.getFreeActive(this._logicChair).put(active.node);
                // active.destroy();
            }
        }
        while(temp.length > 0) {
