...
@ccclass
export default class MGMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("MGMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_MGMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new MGMJEvent(MGMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_MGMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new MGMJEvent(MGMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(MGMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        //this.img_card.spriteFrame=MGMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
