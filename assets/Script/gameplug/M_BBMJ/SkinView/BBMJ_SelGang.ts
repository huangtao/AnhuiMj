...
@ccclass
export default class BBMJ_SelGang extends cc.Component {
    ...
    private Gang_1(){
        ...
        this.img_0.dispatchEvent(new BBMJEvent(BBMJEvent.msg_gangCard, this._gangCard[0]));}
        this.node.active = false;
        this.node.destroy();
    }
    ...
    private Gang_2(){
        if(cc.isValid(this.node)){
        this.img_1.dispatchEvent(new BBMJEvent(BBMJEvent.msg_gangCard, this._gangCard[1]));}
        this.node.active = false;
        this.node.destroy();
    }
    ...
    private Gang_3(){
        if(cc.isValid(this.node)){
        this.img_2.dispatchEvent(new BBMJEvent(BBMJEvent.msg_gangCard, this._gangCard[2]));}
        this.node.active = false;
        this.node.destroy();
    }
    ...
    public showGang(gangCard: Array<number>): void {
        // this.node.removeFromParent(false);
        // M_BBMJView.ins.node.addChild(this.node);
        this._gangCard.splice(0, this._gangCard.length);
        for (var i: number = 0; i < gangCard.length; i++) {
            if (BBMJMahjongDef.gInvalidMahjongValue != gangCard[i]) {
        ...
        if (this._gangCard.length > 1) {
           ...
           // this.img_2.active = (3 == this._gangCard.length);
            ...
            if(3 == this._gangCard.length){
                // url=BBMJ.ins.iclass.getMahjongResName(this._gangCard[2]);
                // SetTextureRes(url,this.img_2);
                for(let i=0;i<this.img_gangcard_2.length;i++){
                    this.img_gangcard_2[i].spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[2]);
                }
                //this.img_2.spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[2]);
            this.btn_gang[0].node.active = true;
            this.btn_gang[1].node.active = true;
          ...
          //  this.node.x=0-this.gang_bg.node.width/2;
            ...
            // url=BBMJ.ins.iclass.getMahjongResName(this._gangCard[0]);
            // SetTextureRes(url,this.img_0);
            // this.img_0.spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[0]);
            for(let i=0;i<this.img_gangcard_0.length;i++){
                this.img_gangcard_0[i].spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[0]);
            }
            // url=BBMJ.ins.iclass.getMahjongResName(this._gangCard[1]);
            // SetTextureRes(url,this.img_1);
            // this.img_1.spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[1]);
            for(let i=0;i<this.img_gangcard_1.length;i++){
                this.img_gangcard_1[i].spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(this._gangCard[1]);
            }
