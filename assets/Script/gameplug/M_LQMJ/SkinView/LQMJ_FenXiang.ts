...
import LQMJ_SinglePlayerFX from "./LQMJ_SinglePlayerFX";
...
@ccclass
export default class LQMJ_FenXiang extends cc.Component {
    ...
    //单个玩家分享信息
    @property([LQMJ_SinglePlayerFX])
    playAry: LQMJ_SinglePlayerFX[]=[];
  ...
  //明杠次数
   ...
   img_head:cc.Sprite[] = [];
    ...
    public init() {
        ...
        // for (var n: number = 0; n < LQMJMahjongDef.gPlayerNum; n++) {
        //     if(0 == n){
    ...
    //         M_LQMJView.ins.GameJiFenBan.node.active = true;
    //     }, 0.3);  
    // }
    ...
    public share(){
        ...
        this.btn_share.scheduleOnce(p=>{
            M_LQMJClass.ins.ScreenCapture(true,this.node);
        }, 0.3);   
    }
    ...
    public copyScore(){
        ...
        M_LQMJClass.ins.CopyToClipboard(scoreStr);
    }
    ...
    public startShow(fun: () => void = null, obj: any = null): void {
        ...
        for (let i = 0; i < LQMJMahjongDef.gPlayerNum; i++) {
            ...
            if(null != M_LQMJClass.ins.TablePlayer[i])
                LoadHeader(M_LQMJClass.ins.TablePlayer[i].FaceID, this.img_head[i]);
        }
        M_LQMJVoice.PlayCardType(`/sound/final_report.mp3`);
        this.onShow();
    }
    ...
    protected onShow(): void {
        //播放游戏结束音效
       // M_LQMJVoice.GameOver();
        this.node.active = true;
        this.lab_other.string="临泉麻将";
        //游戏规则
        var rule = M_LQMJClass.ins.GameRule;
        var rule0 = "";
        var rule1 = "";
        ...
        this.LQMJClass.sendData(queryRecord);
    ...
    private loadRecordData(): void {
        let time = new Date();
        if(M_LQMJClass.ins.TableConfig.tableWhere > 0){//亲友圈
            this.lal_club.node.active = true;
            this.lal_clubNum.node.active = true;
            this.lal_clubNum.string = M_LQMJClass.ins.TableConfig.tableWhere.toString();
            this.lal_room.string="亲友圈房号:"+this.LQMJClass.getTableConfig().TableCode;
        }else{
            this.lal_club.node.active = false;
            this.lal_clubNum.node.active = false;
            this.lal_room.string="房间号:"+this.LQMJClass.getTableConfig().TableCode;
        }
        ...
        this.lal_jushu.string=this.LQMJClass.getTableConfig().alreadyGameNum.toString();
