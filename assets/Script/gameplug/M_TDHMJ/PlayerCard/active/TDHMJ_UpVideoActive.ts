...
@ccclass
export default class TDHMJ_UpVideoActive extends TDHMJ_VideoActiveBase {
    ...
    protected refreshHandCard(): void {
        //起始位置
        var startPos: number = TDHMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];

        //开始排版
        
        if(M_TDHMJVideoClass.ins.is2D()){
            //起始位置
            let startPos: number = TDHMJ_UpVideoActive.ArrangeStartPos_lie[this.fixedCardNum];
            ...
            for(let i: number = 0;i < this._cardData.length;i++){
                ...
                this._cardData[i].showCard(this._handCard[i],true,i+1+this.fixedCardNum*3,M_TDHMJVideoClass.ins);
    ...
    public holdTricksCard(holdIdx: number): number {
        ...
        for(var i: number = 0;i < holdNum;i++) {
            this._handCard.push(M_TDHMJVideoClass.ins.getSelfHandCardData()[i]);
        }
