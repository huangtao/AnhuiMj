...
@ccclass
export default class BBMJ_UpPool extends BBMJ_PoolBase {
    ...
    private resetZ(): void {
        if(this._poolCard.length <= 9){
            for (let i: number = 0; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (i + 21);
            }
        }else if (this._poolCard.length > 9 && this._poolCard.length <= 20) {
            for (let j: number = 0; j < 9; j++) {
                this._poolCard[j].node.zIndex = (j + 21);
            }
            ...
            for (let k: number = 10; k < 20; k++) {
                this._poolCard[k].node.zIndex = (k + 1);
            }
            for (let i: number = 20; i < this._poolCard.length; i++) {
                this._poolCard[i].node.zIndex = (1 - 19);
            }
        }
    ...
    //             this._poolCard[i+20].node.zIndex = (i+1);
    //         }
    //         for(let i: number = 0;i < 10;i++){
    //             this._poolCard[i].node.zIndex = (i+1+temp);
    //         }
    ...
    //         for(let i: number = 0;i < temp;i++){
    //             this._poolCard[i+10].node.zIndex = (i+1);
    //         }
    //         for(let i: number = 0;i < 10;i++){
    //             this._poolCard[i].node.zIndex = (i+1+temp);
    //         }
