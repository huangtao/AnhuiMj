...
@ccclass
export default class BBMJ_QiangGangView extends cc.Component {
    ...
    //     console.log("BBMJ_QiangGangViewSkin");
    ...
    public opHu(){
        ...
        this.btn_hu.scheduleOnce(p=>{
            if (M_BBMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new BBMJEvent(BBMJEvent.msg_qiangGang, 1));
            }
            this.node.active = false;
        }, 0.5);
    ...
    public giveUpOp(){
        ...
        this.btn_hu.scheduleOnce(p=>{
        if (M_BBMJClass.ins.ifCanQiangGang) {
                this.node.dispatchEvent(new BBMJEvent(BBMJEvent.msg_qiangGang, 0));
            }
        this.node.active = false;
        }, 0.5);
    }
    ...
    public showQiangGang(cardValue: number): void {
        ...
        // SetTextureRes(BBMJ.ins.iclass.getMahjongResName(cardValue),this.img_card);
        this.img_card.spriteFrame=BBMJ.ins.iclass.getMahjongPaiHuaRes(cardValue);
    }
}
