...
import { LHZMJMahjongDef, enHuCardType } from "../ConstDef/LHZMJMahjongDef";
...
export class LHZMJMahjongAlgorithm1Lhh {
        ...
        private static CardAryIfContain7Pairs(cardAry:Array<number>,have4Num:number):boolean
        {
            ...
            LHZMJMahjongAlgorithm1.sortCardAry(cardAry);
            have4Num = 0;
            is7pairs = true;
        ...
        public static IsQingYiSe(Ary:Array<number>):boolean
        {
            ...
            var colorfirst:number = LHZMJMahjongAlgorithm1.GetMahjongColor(Ary[0]);

            for(var i=0;i<Ary.length;i++)
            {
                if(colorfirst!= LHZMJMahjongAlgorithm1.GetMahjongColor(Ary[i]))
                {
                    isqys = false;
                    break;
                }
        ...
        public static IsHunYiSe(Ary:Array<number>):boolean
        {
            ...
            var colorfirst:number = LHZMJMahjongDef.gInvalidMahjongValue;

            if(!LHZMJMahjongAlgorithm1.OneAryIsContainsOtherAryAtListOneMember(Ary,LHZMJMahjongAlgorithm1.GetDNXB()))//保证牌值列表里至少有东南西北中的一个
            {
                return isqys;
            }

            for(var i=0;i<Ary.length;i++){
                if(LHZMJMahjongAlgorithm1.GetMahjongLogicValue(Ary[i])>34)//出现中发白，春夏秋冬，梅兰竹菊，那么就不可能是混一色
                {
                    return isqys;
                }
                
                if(LHZMJMahjongAlgorithm1.GetMahjongColor(Ary[i])<3)
                {
                    colorfirst = LHZMJMahjongAlgorithm1.GetMahjongColor(Ary[i]);
                    break;
                }
            }
        ...
        public static IsDuiDuiHu(vSrc:Array<number>):boolean
        {
            ...
            LHZMJMahjongAlgorithm1.sortCardAry(ary);

            var cardtype:Array<number> = LHZMJMahjongAlgorithm1.GetCardTypeAryFromCardAry(ary);

            for(var i=0;i< cardtype.length;i++)
            {
                if(LHZMJMahjongAlgorithm1.GetCardNumInCardAry(ary, cardtype[i])!=2)
                {
                    continue;
                }

                LHZMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(ary, cardtype[i], 2);
                ...
                LHZMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(cardtp,cardtype[i],1);

                var isduidh:boolean = true;

                for(var j=0;j< cardtp.length;j++)
                {
                    if(LHZMJMahjongAlgorithm1.GetCardNumInCardAry(ary, cardtp[j])>=3)
                    {
                        continue;
                    }
                    isduidh = false;
                ...
                LHZMJMahjongAlgorithm1.AddZhiDingNumMemberInAry(ary, cardtype[i], 2);
        ...
        public static IsHuKaZhang(Ary:Array<number>, huCard:number):boolean
        {
            ...
            if (Ary == null || Ary.length < 5 || huCard == LHZMJMahjongDef.gInvalidMahjongValue|| LHZMJMahjongAlgorithm1.GetMahjongColor(huCard)>=3
                ||LHZMJMahjongAlgorithm1.GetMahjongValue(huCard)==1||LHZMJMahjongAlgorithm1.GetMahjongValue(huCard)==9)
            {
                return ishudandiao;
            }
            ...
            LHZMJMahjongAlgorithm1.sortCardAry(byary);


            if(!LHZMJMahjongAlgorithm1.IsContainsNumber(byary,huCard-1)||!LHZMJMahjongAlgorithm1.IsContainsNumber(byary,huCard+1)){
                return ishudandiao;
            }

            //下面的逻辑很仓促写出来的，严密性需要检验

            LHZMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard - 1,1);
            LHZMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard,1);
            LHZMJMahjongAlgorithm1.RemoveZhiDingNumMemberInAry(byary,huCard + 1,1);

            if(LHZMJMahjongAlgorithm1.CheckIfCanHuCardArray(byary))
            {
                ishudandiao = true;
            }
        ...
        {
            if (chair == LHZMJMahjongDef.gInvalidChar || chair > 3)
            {
                return LHZMJMahjongDef.gInvalidChar;
            }
            if (chair == 0)
            {
                return 3;
        ...
        public static RemoveOneMember(Ary:Array<number>,card:number):void{
            ...
            if(card==LHZMJMahjongDef.gInvalidMahjongValue){
                return;
            }
            if(!LHZMJMahjongAlgorithm1.AryContainsOneOrNot(Ary,card)){
                return;
            }
            for(var i=0;i<Ary.length;i++){
                if(Ary[i]!=card){
        ...
        public static RemoveMember(Ary:Array<number>,card:number,length:number=3):void
        {
            ...
            for (var i=0;i<2*length-1;i++)
            {
                ...
                if (LHZMJMahjongAlgorithm1.IsContainsNumber(Ary,cd))
                {
        ...
        public static IsContansFlowrCard(CardAry:Array<number>):boolean
        {
            ...
            for(var i=0;i<M_LHZMJClass.ins.FlowerAry.length;i++){
                FlowerAry.push(M_LHZMJClass.ins.FlowerAry[i]);
            }
            for (var i = 0; i < CardAry.length; i++)
            {
                if (LHZMJMahjongAlgorithm1.IsContainsNumber(FlowerAry,CardAry[i]))
                {
                    iscf = true;
                }
            }
         ...
         * @param card 
         */
        public static IsFlowerCardLHZMJ(card:number):boolean{
            var FlowerAry:Array<number> = new Array<number>();
            for(var i=0;i<M_LHZMJClass.ins.FlowerAry.length;i++){
                FlowerAry.push(M_LHZMJClass.ins.FlowerAry[i]);
            }
