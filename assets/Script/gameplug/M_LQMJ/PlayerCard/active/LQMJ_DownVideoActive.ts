...
@ccclass
export default class LQMJ_DownVideoActive extends LQMJ_VideoActiveBase {
    ...
    protected refreshHandCard(): void {
        ...
        if(M_LQMJVideoClass.ins.is2D()){
            //起始位置
            let startPos: number = LQMJ_DownVideoActive.ArrangeStartPos_lie[this.fixedCardNum];
            ...
            for(var i: number = this._cardData.length;i > 0;i--) {
                ...
                this._cardData[i - 1].showCard(this._handCard[idx],true,0,M_LQMJVideoClass.ins);
            ...
            //起始位置
            let startPos: number = LQMJ_DownVideoActive.ArrangeStartPos3D_lie[this.fixedCardNum];
            ...
            for(let i: number = this._cardData.length; i>0; i--){
                ...
                this._cardData[i - 1].showCard(this._handCard[i-1],true,++temp,M_LQMJVideoClass.ins);
