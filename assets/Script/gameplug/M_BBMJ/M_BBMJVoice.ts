import { BBMJSoundDef, BBMJ } from "./ConstDef/BBMJMahjongDef";
import { AudioType, VoiceType } from "../../CustomType/Enum";

const { ccclass, property } = cc._decorator;

@ccclass
export default class M_BBMJVoice{
    ...
    public static PlayChatVoice(value: number, gender: number,Type:VoiceType) {
        ...
        BBMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }else{
        ...
        // var path = "resources/gameres/M_BBMJ/sound/";
        // if (gender == 1) {
    ...
    public static PlayCardType(str:string) {
        var path = BBMJSoundDef.VoicePath;
        // if (gender == 1) {
    ...
    public static PlayPublicSound(value: string) {
        var path = cc.url.raw(BBMJSoundDef.CommonVoicePath + "Voice/" + value + ".mp3");
        BBMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放系统音效
    ...
    public static PlaySysSound(value: string) {
        var path = cc.url.raw(BBMJSoundDef.VoicePath + "SysSound/Sys76_" + value + ".mp3");
        BBMJ.ins.iclass.playMJSound(path, AudioType.Effect, false);
    }
    /**
     * 播放bgm
    ...
    public static PlayBgm() {
        ...
        BBMJ.ins.iclass.playMJSound(path, AudioType.Music, true);
    }
}
